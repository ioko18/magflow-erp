...
async def test_emag_connection():
    ...
    main_base_url = os.getenv("EMAG_MAIN_BASE_URL", "https://marketplace-api.emag.ro/api-3")
            ...
            if isinstance(products_response, dict):
                ...
                if "results" in products_response:
                    ...
                    if products:
                        ...
                        print(f"💰 Preț: {first_product.get('price', 'N/A')} {first_product.get('currency', 'RON')}")
                else:
    ...
    fbe_base_url = os.getenv("EMAG_FBE_BASE_URL", "https://marketplace-api.emag.ro/api-3")
