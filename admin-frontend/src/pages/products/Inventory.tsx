...
import {
  ...
  Empty, message as antMessage, Badge, Progress, InputNumber, Tooltip
} from 'antd';
import {
  ...
  DashboardOutlined, FilterOutlined, ClearOutlined, EditOutlined, SaveOutlined
  ...
  const columns: ColumnsType<LowStockProduct> = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        const isEditing = editingReorder.has(inventoryItemId);
        const isSaving = savingReorder.has(inventoryItemId);
        const editValue = editingReorder.get(inventoryItemId) ?? (record.reorder_point || 0);
        ...
        return (
          <Space direction="vertical" size={4} style={{ width: '100%' }}>
              ...
              {isEditing ? (
                <>
                  <InputNumber
                    ...
                    onChange={(value) => {
                      if (value !== null) {
                        setEditingReorder(prev => new Map(prev).set(inventoryItemId, value));
                      }
                    }}
                    style={{ width: 60 }}
                    disabled={isSaving}
                  ...
                  <Button
                    ...
                    size="small"
                    icon={<SaveOutlined />}
                    onClick={() => handleUpdateReorderPoint(inventoryItemId, editValue)}
                    loading={isSaving}
                    style={{ padding: '0 6px' }}
                  />
                  <Button
                ...
                <>
                  <Text strong style={{ color: '#1890ff' }}>{record.reorder_point || 0}</Text>
                  <Tooltip title="Edit reorder point">
                    <Button
                      ...
                      icon={<EditOutlined />}
                      onClick={() => {
                        setEditingReorder(prev => new Map(prev).set(inventoryItemId, record.reorder_point || 0));
                      }}
                      style={{ padding: '0 4px' }}
                    />
                  </Tooltip>
            ...
            <Space size={4} style={{ width: '100%' }}>
              ...
              {editingReorderQty.has(inventoryItemId) ? (
                <>
                  <InputNumber
                    size="small"
                    min={0}
                    max={10000}
                    value={editingReorderQty.get(inventoryItemId) ?? (record.reorder_quantity || 0)}
                    onChange={(value) => {
                      if (value !== null) {
                        setEditingReorderQty(prev => new Map(prev).set(inventoryItemId, value));
                      }
                    }}
                    style={{ width: 60 }}
                    disabled={savingReorderQty.has(inventoryItemId)}
                  />
                  <Button
                    type="primary"
                    size="small"
                    ...
                    onClick={() => handleUpdateReorderQty(
                      inventoryItemId, 
                      editingReorderQty.get(inventoryItemId) ?? (record.reorder_quantity || 0)
                    )}
                    loading={savingReorderQty.has(inventoryItemId)}
                    style={{ padding: '0 6px' }}
                  />
                  <Button
                    size="small"
                ...
                <>
                  <Text strong style={{ color: '#52c41a' }}>{record.reorder_quantity || 0}</Text>
                  {record.manual_reorder_quantity !== null && record.manual_reorder_quantity !== undefined && (
                    <Tag color="blue" style={{ fontSize: 10 }}>Manual</Tag>
                  )}
                  <Tooltip title={record.manual_reorder_quantity !== null && record.manual_reorder_quantity !== undefined
                    ...
                    <Button
                      ...
                      onClick={() => {
                        setEditingReorderQty(prev => new Map(prev).set(
                          inventoryItemId, 
                          record.manual_reorder_quantity ?? (record.reorder_quantity || 0)
                        ));
                      }}
                      style={{ padding: '0 4px' }}
                    />
                  ...
                  {record.manual_reorder_quantity !== null && record.manual_reorder_quantity !== undefined && (
                    <Tooltip title="Reset to automatic calculation">
                      <Button
                        ...
                        size="small"
                        danger
                        onClick={() => handleUpdateReorderQty(inventoryItemId, null)}
                        loading={savingReorderQty.has(inventoryItemId)}
