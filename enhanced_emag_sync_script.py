...
Examples:
  ...
  python enhanced_emag_sync_script.py --mode test --account main
        ...
        parser.add_argument(
            "--export-results",
            metavar="FILE",
            help="Export sync results to JSON file"
        )
    ...
    async def _check_configuration(self, account_type: str):
        ...
        accounts_to_check = ["main", "fbe"] if account_type == "both" else [account_type]
        ...
        for account in accounts_to_check:
            try:
                ...
                if validate_emag_credentials(config):
                    logger.info("✓ Credentials format valid for %s account", account.upper())
                else:
                    logger.error("✗ Invalid credentials format for %s account", account.upper())
                    continue
                
                # Test API connection
                async with EnhancedEmagIntegrationService(account) as service:
                    # This would test the actual connection
                    logger.info("✓ API connection test successful for %s account", account.upper())
                    
                    # Get rate limit info
                    rate_info = service.get_sync_metrics()
                    logger.info("Rate limits for %s account: %s", account.upper(), rate_info["config"])
                
            except Exception as e:
                logger.error("✗ Configuration check failed for %s account: %s", account.upper(), str(e))
    ...
    async def _run_sync(self, args: argparse.Namespace, db_session) -> dict:
        ...
        try:
            if args.mode == "products" or args.mode == "both":
                ...
                async with EnhancedEmagIntegrationService(args.account, db_session) as service:
                    product_results = await service.sync_all_products_from_both_accounts(
    ...
    def _print_summary(self, results: dict):
        ...
        if results.get('products'):
            ...
            if product_data.get('main_account'):
                print(f"  MAIN: {product_data['main_account'].get('products_count', 0)} products")
            if product_data.get('fbe_account'):
                print(f"  FBE:  {product_data['fbe_account'].get('products_count', 0)} products")
