...
def test_config():
    ...
    print(f"  Valid: {bool(main_username and main_password and len(main_username) > 3 and len(main_password) > 6)}")
    ...
    print(f"  Valid: {bool(fbe_username and fbe_password and len(fbe_username) > 3 and len(fbe_password) > 6)}")
    ...
    # Test API settings
    print(f"\nAPI Settings:")
    print(f"  Base URL: {os.getenv('EMAG_MAIN_BASE_URL', 'https://marketplace-api.emag.ro/api-3')}")
...
async def test_api_connection():
    ...
    print("ℹ️  Note: Actual API connection test requires real credentials and network access")
...
def main():
    ...
    if config_valid:
        ...
        print("4. Test sync: python enhanced_emag_sync_script.py --mode products --account both --max-pages 2")
    else:
