...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "manufacturer_info", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
            "eu_representative", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
        ...
        sa.Column(
            "raw_emag_data", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.CheckConstraint(
    ...
    op.create_index(
        "idx_emag_products_sku_account", "emag_products", ["sku", "account_type"]
    )
    ...
    op.create_index(
        "idx_emag_products_last_synced", "emag_products", ["last_synced_at"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "shipping_size", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
    ...
    op.create_index(
        "idx_emag_offers_sku_account", "emag_product_offers", ["sku", "account_type"]
    )
    op.create_index("idx_emag_offers_emag_id", "emag_product_offers", ["emag_offer_id"])
    op.create_index(
        "idx_emag_offers_sync_status", "emag_product_offers", ["sync_status"]
    )
    op.create_index("idx_emag_offers_status", "emag_product_offers", ["status"])
    op.create_index(
        "idx_emag_offers_last_synced", "emag_product_offers", ["last_synced_at"]
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "payment_mode_id IN (1,2,3)", name="ck_emag_orders_payment_mode"
        ),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_table(
        ...
        sa.Column(
            "sync_params", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column("status", sa.String(length=50), nullable=False),
    ...
    op.create_index(
        "idx_emag_sync_logs_type_account",
        "emag_sync_logs",
        ["sync_type", "account_type"],
    )
    ...
    op.create_index(
        "idx_emag_sync_progress_log_id", "emag_sync_progress", ["sync_log_id"]
    )
    op.create_index(
        "idx_emag_sync_progress_active", "emag_sync_progress", ["is_active"]
    )

    # Set default values for existing columns
    op.execute(
        "UPDATE emag_products SET account_type = 'main' WHERE account_type IS NULL"
    )
    ...
    op.execute(
        "UPDATE emag_products SET images_overwrite = false WHERE images_overwrite IS NULL"
    )
    op.execute(
        "UPDATE emag_products SET sync_status = 'pending' WHERE sync_status IS NULL"
    )
