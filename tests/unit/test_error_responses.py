...
def test_validation_error() -> None:
    ...
    with pytest.raises(httpx.ConnectError):
        ...
        assert data["title"] == "Validation Error" and data["status"] == 422, \
...
def test_not_found() -> None:
    ...
    with pytest.raises(httpx.ConnectError):
        ...
        assert data["title"] == "Not Found" and data["status"] == 404, \
            f"Incorrect error details: {data}"
...
def test_unauthorized() -> None:
    ...
    with pytest.raises(httpx.ConnectError):
        ...
        assert "www-authenticate" in response.headers, "WWW-Authenticate header is missing"
        ...
        assert data["title"] == "Unauthorized" and data["status"] == 401, \
            f"Incorrect error details: {data}"
