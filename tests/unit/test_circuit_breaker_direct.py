...
async def test_failure_threshold():
    ...
    assert status["failures"] >= FAILURE_THRESHOLD  # Could be more if previous tests left failures
...
async def test_half_open_state():
    ...
    assert status["status"] == "healthy"  # half-open is considered healthy for monitoring
    ...
    assert status["failures"] == 0, "Failure count should be reset after successful close"
    assert status["status"] == "healthy"
...
async def test_failure_in_half_open_state():
    ...
    circuit_breaker.record_failure()
    status = await get_circuit_status(TEST_CIRCUIT_NAME)
    assert status["state"] == "open", f"Expected OPEN after failure in half-open, got {status['state']}"
    assert status["status"] == "unhealthy"
...
async def test_circuit_breaker_error_handling():
    ...
    with pytest.raises(ValueError, match="failure_window_seconds must be greater than 0"):
