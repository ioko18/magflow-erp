...
async def test_health_check_unhealthy():
    ...
    assert status["metadata"]["circuit_breakers"]["test_unhealthy"]["status"] == "unhealthy"
...
async def test_health_check_half_open():
    ...
    assert status["status"] == "healthy"
    assert status["metadata"]["circuit_breakers"]["test_half_open"]["state"] == "half-open"
...
async def test_health_check_multiple_breakers():
    ...
    assert status["metadata"]["circuit_breakers"]["test_healthy_1"]["status"] == "healthy"
    assert status["metadata"]["circuit_breakers"]["test_unhealthy_1"]["status"] == "unhealthy"
...
async def test_health_check_performance():
    ...
    assert num_breakers == len(batched_status["metadata"]["circuit_breakers"]), \
