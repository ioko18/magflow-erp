    ...
    async for db in get_async_session():
        ...
        if products:
            ...
            for i, product in enumerate(products[:3]):
                print(f"  {i+1}. {product.sku} - {product.name[:50]}... ({product.account_type})")
        else:
            # Să verific ce produse există în general
            result2 = await db.execute(text("SELECT COUNT(*), account_type FROM app.emag_products_v2 GROUP BY account_type"))
            ...
            result3 = await db.execute(text("SELECT COUNT(*), is_active FROM app.emag_products_v2 GROUP BY is_active"))
