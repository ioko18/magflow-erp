...
def test_endpoint(url, expected_status, test_name, method="GET", data=None):
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            ...
            if response.status == 405:
                if (
                    ...
                    print(
                        f"❌ FAIL: Expected content type to include 'application/json' or 'application/problem+json', got '{content_type}'"
                    )
                    return False
            ...
            if expected_status == 401 and response.status == 401:
                if "application/json" in content_type:
                    ...
                    print(
                        f"❌ FAIL: Expected content type to include 'application/json' for 401, got '{content_type}'"
                    )
                    return False
            ...
            if (
                ...
                print(
                    f"❌ FAIL: Expected content type to include 'application/problem+json' or 'application/json' for status {expected_status}, got '{content_type}'"
                )
                return False
        ...
        # For all other errors, expect application/problem+json
            ...
            print(
                f"❌ FAIL: Expected content type to include 'application/problem+json', got '{content_type}'"
            )
            return False
