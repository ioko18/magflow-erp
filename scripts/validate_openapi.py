...
class OpenAPIValidator:
    ...
    def _add_issue(self, path: str, message: str, level: str = "error") -> None:
        ...
        self.issues.append({"path": path, "message": message, "level": level})
    ...
    def _check_operation(
        ...
        if not operation.get("summary"):
            self._add_issue(
                f"{op_path}.summary", "Operation summary is missing", "warning"
            )

        if not operation.get("description"):
            self._add_issue(
                f"{op_path}.description", "Operation description is missing", "warning"
            )
    ...
    def _check_request_body(self, op_path: str, request_body: Dict[str, Any]) -> None:
        ...
        if not request_body.get("description"):
            self._add_issue(
                f"{body_path}.description",
                "Request body description is missing",
                "warning",
            )
    ...
    def _check_response(
        ...
        if not response.get("description"):
            self._add_issue(
                f"{resp_path}.description", "Response description is missing"
            )
    ...
    def validate_against_spec(self) -> bool:
        ...
        try:
            ...
            self._add_issue(
                "", f"Failed to validate against OpenAPI 3.1.1 schema: {str(e)}"
            )
            return False
