                        ...
                        if response.status == 200:
                            ...
                            if not result.get("isError", True):
                                ...
                                if page >= pagination.get("totalPages", 0):
                                    print(f"    ℹ️  Reached last page ({pagination.get('totalPages', 0)})")
                                    break
                            else:
...
async def test_emag_sync(account_type: str = "main", max_pages: int = 2):
    ...
    if connection_result["success"]:
        ...
        if isinstance(response, dict):
            if not response.get("isError", True):
                ...
                print(f"   ℹ️  Total items available: {pagination.get('totalItems', 'unknown')}")
                print(f"   ℹ️  Total pages available: {pagination.get('totalPages', 'unknown')}")
            else:
                error_msg = response.get("messages", [{"message": "Unknown error"}])[0].get("message", "Unknown error")
    ...
    if products:
        ...
        print(f"     Price: {sample_product.get('price', 'N/A')} {sample_product.get('currency', 'RON')}")
