...
async def collect_database_metrics(session: AsyncSession, use_cache: bool = True) -> DatabaseMetrics:
    ...
    try:
        async with asyncio.timeout(HEALTH_CHECK_TIMEOUT):
            if is_sqlite:
                ...
                try:
                    ...
                    if cache_row:
                        metrics['cache_hit_ratio'] = float(cache_row.hit_ratio or 0) * 100
                    
                except Exception as e:
                    logger.error("Error collecting PostgreSQL metrics: %s", str(e), exc_info=True)
        ...
        logger.warning("Database metrics collection timed out after %s seconds", HEALTH_CHECK_TIMEOUT)
        metrics['error'] = f"Metrics collection timed out after {HEALTH_CHECK_TIMEOUT} seconds"
        return metrics
    except Exception as e:
        logger.error("Unexpected error collecting database metrics: %s", str(e), exc_info=True)
        metrics['error'] = f"Unexpected error: {str(e)}"
        return metrics

async def get_database_health_status(session: Optional[AsyncSession] = None) -> Dict[str, Any]:
...
async def _get_postgresql_health_status(session: AsyncSession) -> Dict[str, Any]:
    ...
    try:
        ...
        if max_connections and active_connections > max_connections * 0.9:
            issues.append(f"High connection utilization: {active_connections}/{max_connections} connections in use")
            is_healthy = False
        ...
        return {
            ...
            'metrics': {
                ...
                'connection_utilization': (active_connections / max_connections * 100) if max_connections else 0,
        ...
        logger.error("Error getting PostgreSQL health status: %s", str(e), exc_info=True)
        return {
            'status': 'unhealthy',
            'error': str(e),
            'timestamp': datetime.utcnow().isoformat()
...
def setup_pool_monitoring():
    ...
    def on_checkin(dbapi_connection, connection_record):
        ...
        if hasattr(connection_record, '_checkout_time'):
            ...
            pool_metrics['avg_wait_time'] = pool_metrics['wait_time'] / pool_metrics['checkins']
    ...
    def on_error(exception_context):
        ...
        pool_metrics['last_updated'] = datetime.utcnow().isoformat()
        logger.error("Database error: %s", str(exception_context.original_exception), 
...
async def main():
    ...
    try:
        ...
        async with get_async_session() as session:
            ...
            metrics = await collect_database_metrics(session)
            print("\nDatabase Metrics:")
            print("- Active Connections: {}".format(metrics.get('active_connections', 'N/A')))
            print("- Connection Utilization: {:.1f}%".format(metrics.get('connection_utilization', 0)))
