...
class StartupProbeResponse(BaseModel):
    ...
    status: str = Field(..., description="Current status of the service during startup")
    message: str = Field(..., description="Detailed message about the startup status")
    timestamp: datetime = Field(..., description="When the startup check was performed")
    startup_time: datetime = Field(..., description="When the service started up")
