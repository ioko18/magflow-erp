...
class CharacteristicValue(BaseModel):
    ...
    tag: Optional[str] = Field(None, description="Tag for size characteristics: 'original' or 'converted'")
...
class ImageData(BaseModel):
    ...
    overwrite: Optional[bool] = Field(False, description="Overwrite existing image")
    force_download: Optional[bool] = Field(False, description="Force download from URL")
...
class DraftProductCreate(BaseModel):
    ...
    part_number: str = Field(..., min_length=1, max_length=100, description="Manufacturer part number")
    category_id: Optional[int] = Field(None, description="eMAG category ID")
    ean: Optional[List[str]] = Field(None, description="List of EAN codes")
...
class CompleteProductCreate(BaseModel):
    ...
    part_number: str = Field(..., min_length=1, max_length=100, description="Manufacturer part number")
    brand: str = Field(..., min_length=1, max_length=100, description="Brand name")
    description: str = Field(..., min_length=10, description="Product description (HTML allowed)")

    # Images
    images: List[ImageData] = Field(..., min_items=1, description="Product images (at least 1 required)")

    # Characteristics with tag support
    characteristics: List[CharacteristicValue] = Field(..., description="Product characteristics")
    ...
    handling_time: List[HandlingTimeData] = Field(..., min_items=1, description="Handling time per warehouse")
    ...
    manufacturer: Optional[GPSRManufacturer] = Field(None, description="Manufacturer information (GPSR)")
    eu_representative: Optional[GPSREURepresentative] = Field(None, description="EU representative (GPSR)")
    safety_information: Optional[str] = Field(None, description="Safety information (GPSR)")
...
class AttachOfferCreate(BaseModel):
    ...
    product_id: int = Field(..., description="Seller internal product ID")
    part_number_key: Optional[str] = Field(None, description="eMAG part number key")
    ean: Optional[List[str]] = Field(None, description="List of EAN codes")
    sale_price: float = Field(..., gt=0, description="Sale price")
    vat_id: int = Field(..., description="VAT rate ID")
    stock: List[StockData] = Field(..., min_items=1, description="Stock per warehouse")
...
class BatchProductUpdate(BaseModel):
    ...
    """
    products: List[Dict[str, Any]] = Field(..., max_items=100, description="List of products to update (max 100)")
