...
class RateLimitError(EmagApiError):
    ...
    def __init__(self, remaining_seconds: int = 60, message: str | None = None):
        self.remaining_seconds = remaining_seconds
        msg = message or f"Rate limit exceeded. Retry in {remaining_seconds}s"
        super().__init__(message=msg, code="RATE_LIMIT_EXCEEDED", status_code=429)
...
class NetworkError(EmagApiError):
    ...
    def __init__(self, message: str, original_error: Exception | None = None):
        ...
        super().__init__(message=message, code="NETWORK_ERROR", status_code=503)
...
def retry_on_error(
    max_retries: int = 3, base_delay: float = 2.0, exponential: bool = True
):
    """
...
class ErrorHandler:
    ...
    @staticmethod
    def get_retry_delay(
        error: Exception, attempt: int, base_delay: float = 2.0
    ) -> float:
        """
...
def log_error(error: EmagApiError, context: dict | None = None):
    """
