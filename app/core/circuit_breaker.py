...
class CircuitBreaker:
        ...
        with self._lock:
            ...
            if self._state == CircuitState.OPEN:
                if self._opened_at is not None and \
                   current_time - self._opened_at >= self.recovery_timeout:
                    self._set_half_open()
            ...
            # Check if we should transition from HALF_OPEN to CLOSED or re-OPEN
                ...
                elif self._opened_at is not None and (current_time - self._opened_at) >= (self.recovery_timeout * 2):
                    self._logger.debug(
            ...
            elif self._state == CircuitState.CLOSED and len(self._failure_timestamps) >= self.failure_threshold:
                self._trip_open()
    ...
    def _emit_state_change_event(self, from_state: CircuitState, to_state: CircuitState) -> None:
...
def get_circuit_breaker(name: str, **kwargs: Any) -> CircuitBreaker:
    ...
    with _registry_lock:
        ...
        if name == "database" and name not in _registry:
            _logger.debug(f"Returning global DATABASE_CIRCUIT_BREAKER instance: {id(DATABASE_CIRCUIT_BREAKER)}")
            return DATABASE_CIRCUIT_BREAKER
            
        if name in _registry:
            cb = _registry[name]
            _logger.debug(f"Returning existing circuit breaker '{name}': {id(cb)}, state: {cb._state}")
            return cb
        ...
        _logger.debug(f"Created new circuit breaker '{name}': {id(cb)}, state: {cb._state}")
        _registry[name] = cb
        return cb
