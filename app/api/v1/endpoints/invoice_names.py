...
class GenerateInvoiceNameRequest(BaseModel):
    ...
    sku: str
    max_length: int = Field(50, ge=20, le=200, description="Maximum length for generated names")
...
async def list_products_with_invoice_names(
    has_invoice_names: Optional[bool] = Query(None, description="Filter by presence of invoice names"),
    ...
    if has_invoice_names is True:
        query = query.where(
            (Product.invoice_name_ro.isnot(None)) | (Product.invoice_name_en.isnot(None))
        )
...
async def update_invoice_names(
    sku: str,
    invoice_name_ro: Optional[str] = Query(None, max_length=200),
    invoice_name_en: Optional[str] = Query(None, max_length=200),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """
