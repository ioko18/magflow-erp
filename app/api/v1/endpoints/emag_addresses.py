...
class AddressResponse(BaseModel):
    ...
    address_type_id: int = Field(..., description="1=Return, 2=Pickup, 3=Invoice HQ, 4=Delivery estimates")
    ...
    quarter: Optional[str] = Field(None, description="Quarter/district")
...
class AWBRequest(BaseModel):
    ...
    order_id: Optional[int] = Field(None, description="eMAG order ID (for orders)")
    rma_id: Optional[int] = Field(None, description="RMA ID (for returns)")
    sender_address_id: Optional[str] = Field(None, description="Saved sender address ID (NEW v4.4.9)")
    sender_name: Optional[str] = Field(None, description="Sender name")
    sender_contact: Optional[str] = Field(None, description="Sender contact person")
    sender_phone1: Optional[str] = Field(None, description="Sender primary phone")
    sender_locality_id: Optional[int] = Field(None, description="Sender locality ID")
    sender_street: Optional[str] = Field(None, description="Sender street address")
    receiver_address_id: Optional[str] = Field(None, description="Saved receiver address ID (NEW v4.4.9)")
    receiver_name: Optional[str] = Field(None, description="Receiver name")
    receiver_contact: Optional[str] = Field(None, description="Receiver contact person")
    receiver_phone1: Optional[str] = Field(None, description="Receiver primary phone")
    receiver_locality_id: Optional[int] = Field(None, description="Receiver locality ID")
    receiver_street: Optional[str] = Field(None, description="Receiver street address")
    receiver_legal_entity: int = Field(0, description="Whether receiver is legal entity")
    ...
    date: Optional[str] = Field(None, description="Pickup date for returns (YYYY-MM-DD)")
    ...
    **Example Response:**
    ...
    try:
        async with await get_emag_client(account_type) as client:
            ...
            response = await client.get_addresses(page=page, items_per_page=items_per_page)

            if response.get("isError"):
                error_messages = response.get("messages", [])
                error_text = error_messages[0] if error_messages else "Unknown error"
                raise HTTPException(status_code=400, detail=f"eMAG API error: {error_text}")
...
async def get_pickup_addresses(
    ...
    try:
        async with await get_emag_client(account_type) as client:
            ...
            for addr in results:
                if addr.get("address_type_id") == 2:  # Pickup address
                    pickup_addresses.append(AddressResponse(
                        address_id=str(addr.get("address_id", "")),
                        country_id=addr.get("country_id", 0),
                        country_code=addr.get("country_code", ""),
                        address_type_id=addr.get("address_type_id", 0),
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch pickup addresses: {str(e)}")
...
async def get_return_addresses(
    ...
    try:
        async with await get_emag_client(account_type) as client:
            ...
            for addr in results:
                if addr.get("address_type_id") == 1:  # Return address
                    return_addresses.append(AddressResponse(
                        address_id=str(addr.get("address_id", "")),
                        country_id=addr.get("country_id", 0),
                        country_code=addr.get("country_code", ""),
                        address_type_id=addr.get("address_type_id", 0),
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch return addresses: {str(e)}")
