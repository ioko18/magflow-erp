...
class CreateLocalVariantRequest(BaseModel):
    ...
    new_ean: Optional[str] = Field(None, description="EAN code (if different)")
...
async def create_local_variant(
    ...
    new_product = Product(
        ...
        manufacturer=request.manufacturer or (original.manufacturer if original else None),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to create variant relationship: {str(e)}"
        )
