...
class AWBBulkGenerateRequest(BaseModel):
    ...
    account_type: str = Field(..., description="Account type (main or fbe)")
    courier_account_id: int = Field(..., description="Courier service ID")
    orders: List[Dict[str, Any]] = Field(..., description="List of orders with order_id and optional packages")
...
class EANMatchRequest(BaseModel):
    ...
    product_data: Optional[Dict[str, Any]] = Field(None, description="Optional local product data")


class InvoiceGenerateRequest(BaseModel):
    """Request model for invoice generation."""
    account_type: str = Field(..., description="Account type (main or fbe)")
    invoice_url: Optional[str] = Field(None, description="Optional pre-generated invoice URL")
...
async def get_courier_accounts(
    ...
    logger.info("User %s fetching courier accounts for %s", current_user.email, account_type)
...
async def bulk_generate_awbs(
    ...
    logger.info("User %s bulk generating AWBs for %d orders", current_user.email, len(request.orders))
...
async def get_invoice_data(
    ...
    logger.info("User %s generating invoice data for order %d", current_user.email, order_id)
...
async def bulk_generate_invoices(
    ...
    logger.info("User %s bulk generating invoices for %d orders", current_user.email, len(request.order_ids))
