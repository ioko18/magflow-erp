...
class CustomerResponse(BaseModel):
    ...
    emag_customer_id: str | None = None
    emag_orders_count: int | None = None
...
async def get_emag_customers(
    ...
    status: str | None = Query(
        default=None, description="Filter by status: active, inactive, blocked"
    ),
    ...
    try:
        ...
        async for async_db in get_async_session():
            ...
            result = await async_db.execute(
                text(customers_query), {"limit": limit, "skip": skip}
            )
            ...
            for row in customers_data:
                ...
                loyalty_score = min(
                    100, int((row.total_orders or 0) * 10 + (total_spent / 100))
                )
