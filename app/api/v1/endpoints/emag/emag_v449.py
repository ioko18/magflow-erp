...
from app.services.emag.emag_validation_service import EmagValidationService
...
async def search_products_by_ean(
    ...
    try:
        logger.info(
            f"EAN search requested by {current_user.email} for {len(request.ean_codes)} codes"
        )
...
async def quick_update_offer(
    ...
    try:
        logger.info(
            f"Quick offer update requested by {current_user.email} for product {product_id}"
        )
        ...
        async with EnhancedEmagIntegrationService(account_type, db) as service:
            ...
            return {
                ...
                "updated_fields": [k for k in offer_data if k != "id"],
...
async def save_product_measurements(
    ...
    try:
        logger.info(
            f"Measurements save requested by {current_user.email} for product {product_id}"
        )
        ...
        stmt = (
            ...
            .where(
                (EmagProductV2.emag_id == product_id)
                | (EmagProductV2.sku == product_id)
            )
            .where(EmagProductV2.account_type == account_type)
        )
        ...
        if not emag_product_id:
            raise HTTPException(
                status_code=400, detail="Product does not have an eMAG ID"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Measurements save failed: {str(e)}"
        )
...
async def update_stock_only(
    ...
    try:
        logger.info(
            f"Stock-only update requested by {current_user.email} for product {product_id}"
        )
        ...
        async with EnhancedEmagIntegrationService(account_type, db) as service:
            ...
            logger.info(
                f"Successfully updated stock for product {product_id} to {stock_value}"
            )
...
class CampaignProposalRequest(BaseModel):
    ...
    max_qty_per_order: int | None = Field(
        None, description="Max quantity per customer order"
    )
    voucher_discount: int | None = Field(
        None, ge=10, le=100, description="Voucher discount percentage (10-100)"
    )
    post_campaign_sale_price: float | None = Field(
        None, description="Price after campaign ends"
    )
    not_available_post_campaign: bool = Field(
        default=False, description="Deactivate offer after campaign"
    )
    date_intervals: list[dict[str, Any]] | None = Field(
        None, description="Required for MultiDeals campaigns"
    )
...
async def propose_to_campaign(
    ...
    try:
        logger.info(
            f"Campaign proposal requested by {current_user.email} for product {product_id} to campaign {proposal.campaign_id}"
        )
        ...
        async with EnhancedEmagIntegrationService(account_type, db) as service:
            ...
            logger.info(
                f"Successfully proposed product {product_id} to campaign {proposal.campaign_id}"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Campaign proposal failed: {str(e)}"
        )
...
class ProductValidationRequest(BaseModel):
    ...
    category_template: dict[str, Any] | None = Field(
        None, description="Optional category template"
    )
...
async def validate_products_bulk(
    ...
    try:
        logger.info(
            f"Bulk validation requested by {current_user.email} for {len(products)} products"
        )
        ...
        for idx, product_data in enumerate(products):
            try:
                is_valid, errors, warnings = (
                    validation_service.validate_product_complete(
                        product_data=product_data, category_template=category_template
                    )
                )
