...
async def get_all_offers(
    status: str | None = Query(None, description="Offer status filter"),
...
async def compare_offers_between_accounts(
    sku: str | None = Query(None, description="Filter by specific SKU"),
    category_id: str | None = Query(None, description="Category ID filter"),
    limit: int = Query(50, description="Items per page", ge=1, le=100),
    current_user: UserModel = Depends(get_current_active_user),
) -> dict[str, Any]:
...
async def search_offers(
    ...
    account_type: EmagAccountType | None = Query(
        None,
    ...
    try:
        ...
        if account_type is None or account_type == EmagAccountType.MAIN:
            ...
            try:
                ...
                logger.error(f"Error searching MAIN account: {e}", exc_info=True)
        ...
        if account_type is None or account_type == EmagAccountType.FBE:
            ...
            try:
                ...
                logger.error(f"Error searching FBE account: {e}", exc_info=True)
    ...
    try:
        ...
        if account_type is None or account_type == EmagAccountType.MAIN:
            ...
            try:
                ...
                logger.error(f"Error getting MAIN offer details: {e}", exc_info=True)
        ...
        if account_type is None or account_type == EmagAccountType.FBE:
            ...
            try:
                ...
                logger.error(f"Error getting FBE offer details: {e}", exc_info=True)
