...
# SECURITY FIX: Validate and sanitize schema name to prevent SQL injection
...
async def get_emag_service() -> EmagIntegrationService | None:
...
async def get_emag_status(
    ...
    emag_service: EmagIntegrationService | None = Depends(get_emag_service),
) -> dict[str, Any]:
...
async def get_duplicate_analysis(
    ...
    """
    try:
        from app.services.emag.emag_analytics_service import EmagAnalyticsService
...
async def setup_scheduled_sync(
    ...
    try:
        ...
        scheduled_sync = {
            ...
            "accounts": accounts,
            "next_sync_time": (
                datetime.now(UTC) + timedelta(minutes=sync_interval)
            ).isoformat(),
            "status": "scheduled",
...
async def get_rate_limit_status(
    user_id: str | None = Query(None, description="User ID for specific status"),
    endpoint: str | None = Query(None, description="Endpoint for specific status"),
    current_user: UserModel = Depends(get_current_active_user),
) -> dict[str, Any]:
...
async def get_all_offers(
    ...
    try:
        async for session in get_async_session():
            ...
            # Build count query with parameterization
            count_query = f"SELECT COUNT(*) FROM {EMAG_PRODUCT_OFFERS_TABLE}"
            count_params = {}

            if normalized_filter:
                count_query += " WHERE account_type = :account_type"
                count_params["account_type"] = normalized_filter
