...
from app.services.emag.enhanced_emag_service import EnhancedEmagIntegrationService
...
class ProductSyncResponse(BaseModel):
    ...
    main_account: dict[str, Any] | None
    fbe_account: dict[str, Any] | None
...
async def get_emag_status(
    ...
    try:
        logger.info(
            "Getting eMAG status for account_type=%s, user=%s",
            account_type,
            current_user.email,
        )
        ...
        async for async_db in get_async_session():
                ...
                recent_syncs.append(
                    {
                        ...
                        "success_rate": round(
                            (processed / total * 100) if total else 0, 2
                        ),
                    }
                )
            ...
            total_syncs = len(
                [s for s in recent_syncs if s["status"] in ["completed", "failed"]]
            )
            successful_syncs = len(
                [s for s in recent_syncs if s["status"] == "completed"]
            )
            health_score = (
                (successful_syncs / total_syncs * 100) if total_syncs > 0 else 100
            )
            ...
            if account_type == "both":
                response["summary"] = {
                    ...
                    "avg_price": round(
                        (main_stats["avg_price"] + fbe_stats["avg_price"]) / 2, 2
                    ),
                    "last_sync": max(
...
class SyncOrdersRequest(BaseModel):
    ...
    status_filter: str | None = Field(
        default=None,
        description="Filter orders by status (1=new, 2=in_progress, 3=prepared, 4=finalized, 5=returned, 0=canceled)",
    )
...
async def update_emag_product(
    ...
    try:
        ...
        if not account_type or account_type not in ["main", "fbe"]:
            raise HTTPException(
                status_code=400,
                detail="account_type is required and must be 'main' or 'fbe'",
            )
        ...
        async for async_db in get_async_session():
            ...
            logger.info(
                f"Successfully updated eMAG product {product_id} ({account_type})"
            )
...
async def get_all_unified_products(
    ...
    source: str = Query(
        default="all", description="Product source: all, emag_main, emag_fbe, local"
    ),
    search: str | None = Query(default=None, description="Search by SKU or name"),
    ...
    try:
                ...
                all_products.append(
                    {
                        ...
                        if hasattr(product, "base_price") and product.base_price
                        else None,
                        "currency": product.currency
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get unified products: {str(e)}"
        )
...
class LightOfferPriceUpdate(BaseModel):
    ...
    recommended_price: float | None = Field(
        default=None, description="Recommended retail price"
    )
    min_sale_price: float | None = Field(
        default=None, description="Minimum sale price"
    )
    max_sale_price: float | None = Field(
        default=None, description="Maximum sale price"
    )
...
class LightOfferBulkUpdate(BaseModel):
    ...
    batch_size: int = Field(
        default=25, ge=1, le=50, description="Batch size (optimal: 25)"
    )
...
async def light_offer_update_price(
    ...
    try:
        ...
        logger.error(
            "Failed to update price via Light Offer API: %s", str(e), exc_info=True
        )
...
async def light_offer_update_stock(
    ...
    try:
        ...
        logger.error(
            "Failed to update stock via Light Offer API: %s", str(e), exc_info=True
        )
    ...
    try:
        ...
        async with EmagLightOfferService(request.account_type) as service:
            result = await service.bulk_update_prices(
                updates=request.updates, batch_size=request.batch_size
            )
        ...
        logger.error(
            "Failed to bulk update prices via Light Offer API: %s",
            str(e),
            exc_info=True,
        )
        raise HTTPException(
            status_code=500, detail=f"Failed to bulk update prices: {str(e)}"
        )
    ...
    try:
        ...
        async for async_db in get_async_session():
            ...
            stock_analysis = {
                ...
                "total_stock": int(stock_row.total_stock)
                if stock_row.total_stock
                else 0,
            }
            ...
            """
            sync_perf_result = await async_db.execute(
                text(sync_perf_query), {"start_date": start_date}
            )
            ...
            sync_performance = {
                ...
                "success_rate": round(
                    (sync_row.successful_syncs / sync_row.total_syncs * 100)
                    if sync_row.total_syncs
                    else 0,
                    2,
                ),
                "avg_duration_seconds": round(float(sync_row.avg_duration), 2)
                if sync_row.avg_duration
                else 0,
...
async def get_emag_vat_rates(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get VAT rates: {str(e)}"
        )
...
async def get_emag_handling_times(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get handling times: {str(e)}"
        )
...
class EanSearchRequest(BaseModel):
    ...
    eans: list[str] = Field(
        ..., max_items=100, description="List of EAN codes to search (max 100)"
    )
    account_type: str = Field(default="main", description="Account type (main or fbe)")
    ...
    **Returns for each EAN:**
    ...
    try:
        if len(request.eans) > 100:
            raise HTTPException(
                status_code=400, detail="Maximum 100 EAN codes per request"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to search by EANs: {str(e)}"
        )
...
class UpdateOfferLightRequest(BaseModel):
    ...
    recommended_price: float | None = Field(
        None, description="Recommended retail price without VAT"
    )
    ...
    stock: list[dict[str, int]] | None = Field(None, description="Stock array")
    handling_time: list[dict[str, int]] | None = Field(
        None, description="Handling time array"
    )
    vat_id: int | None = Field(None, description="VAT rate ID")
    status: int | None = Field(
        None, description="Offer status (0=inactive, 1=active, 2=EOL)"
    )
