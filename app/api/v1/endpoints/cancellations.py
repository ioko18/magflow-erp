...
async def create_cancellation_request(
    ...
    emag_order_id: Optional[str] = None,
...
async def list_cancellation_requests(
    status: Optional[CancellationStatus] = Query(None, description="Filter by status"),
...
async def update_cancellation_status(
    ...
    cancellation_request.approved_at = datetime.utcnow()
...
async def process_cancellation(
    ...
    if process_refund and cancellation_request.refund_amount > 0:
        ...
        cancellation_request.refund_processed_at = datetime.utcnow()
