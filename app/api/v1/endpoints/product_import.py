...
class ImportRequest(BaseModel):
    ...
    auto_map: bool = Field(
        default=True,
        description="Automatically map products to eMAG accounts"
    )
...
class ProductMappingResponse(BaseModel):
    ...
    emag_main_id: Optional[int]
    emag_main_part_number: Optional[str]
    emag_main_status: Optional[str]
    emag_fbe_id: Optional[int]
    emag_fbe_part_number: Optional[str]
    emag_fbe_status: Optional[str]
    mapping_status: str
...
class ProductLookupResponse(BaseModel):
    ...
    sku: str
    matches: Dict[str, Optional[ProductLookupMatch]]
...
@router.post("/import/google-sheets", response_model=ImportResponse)
...
async def lookup_emag_products_by_sku(
    sku: str = Query(..., description="SKU / part number to match against eMAG products"),
    db: AsyncSession = Depends(get_database_session),
    current_user: User = Depends(get_current_user)
):
    ...
    formatted_matches: Dict[str, Optional[ProductLookupMatch]] = {}
...
async def remap_unmapped_products(
    ...
    service = ProductImportService(db)
    summary = await service.remap_unmapped_products(limit=request.limit, dry_run=request.dry_run)
...
async def test_google_sheets_connection(
    ...
    try:
        ...
        if not service.open_spreadsheet():
            raise HTTPException(
                status_code=500,
                detail="Failed to open spreadsheet"
            )
