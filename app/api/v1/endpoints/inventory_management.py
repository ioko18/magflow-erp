    ...
    if status == "out_of_stock":
        ...
        query = query.where(
            and_(
                ...
                (InventoryItem.quantity - InventoryItem.reserved_quantity) <= InventoryItem.minimum_stock
            )
        )
...
async def export_low_stock_to_excel(
    ...
    include_supplier_info: bool = Query(True, description="Include supplier information"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """
    ...
    total_reorder_cost = sum((item[0].unit_cost or 0) * calculate_reorder_quantity(item[0]) for item in items)
...
async def get_inventory_statistics(
    ...
    return {
        ...
        "data": {
            ...
            "stock_health_percentage": round(((in_stock + overstock) / total_items * 100) if total_items > 0 else 0, 2),
        }
    }
