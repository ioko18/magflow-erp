...
class ProductUpdateRequest(BaseModel):
    ...
    chinese_name: Optional[str] = Field(None, max_length=500, description="Chinese product name")
    image_url: Optional[str] = Field(None, max_length=1000, description="Primary product image URL")
    invoice_name_ro: Optional[str] = Field(None, max_length=200, description="Romanian invoice name")
    invoice_name_en: Optional[str] = Field(None, max_length=200, description="English invoice name")
    ean: Optional[str] = Field(None, max_length=18, description="EAN code")
    base_price: Optional[float] = Field(None, ge=0, description="Base price")
    recommended_price: Optional[float] = Field(None, ge=0, description="Recommended price")
    ...
    is_discontinued: Optional[bool] = Field(None, description="Discontinued status")
    change_reason: Optional[str] = Field(None, description="Reason for the change")

    @validator('base_price', 'recommended_price')
...
class BulkUpdateRequest(BaseModel):
    ...
    updates: List[Dict[str, Any]] = Field(..., description="List of product updates with product_id")
...
class ProductDetailResponse(BaseModel):
    ...
    invoice_name_ro: Optional[str]
    invoice_name_en: Optional[str]
    ean: Optional[str]
    base_price: float
        ...
        if hasattr(product, field_name):
            ...
            if field_name == 'sku' and old_value != new_value:
                ...
                if existing_result.scalar_one_or_none():
                    raise HTTPException(
                        status_code=400,
                        detail=f"SKU '{new_value}' already exists"
                    )
...
async def get_sku_history(
    ...
    # Fetch SKU history
    query = select(ProductSKUHistory).where(
        ProductSKUHistory.product_id == product_id
...
async def get_change_log(
    ...
    # Build query
    query = select(ProductChangeLog).where(
        ProductChangeLog.product_id == product_id
    )
...
async def list_products(
    ...
    # Execute query with pagination - disable relationship loading
        ...
        for field, new_value in update_data.items():
            if hasattr(product, field):
                ...
                if old_value != new_value:
                    ...
                    if field == "sku":
                        await log_sku_change(
                            db, product_id, old_value, new_value,
                            current_user.id, None, ip_address, get_user_agent(request)
                        )
                    
                    # Log field change
                    await log_field_change(
                        db, product_id, field, old_value, new_value,
