...
class BulkUpdateRequest(BaseModel):
    ...
    account_type: str | None = Field(None, description="Account type: main or fbe")
...
def validate_product_compliance(
    ...
    invalid_eans = [
        ean for ean in valid_eans if not (6 <= len(ean) <= 14 and ean.isdigit())
    ]
    ...
    if warranty is not None and warranty >= 0:
        add_check(
            "warranty", "Garanție (luni)", "pass", "Garanția produsului este setată."
        )
    else:
        add_check(
            "warranty",
...
async def get_emag_categories(
    category_id: int | None = Query(
        None, description="Specific category ID to fetch"
    ),
...
async def match_products_by_ean(
    ...
    try:
        ...
        if not ean_list:
            return {
                "status": "success",
                "data": {"matches": [], "message": "No valid EAN codes provided"},
            }
...
async def publish_products_to_emag(
    ...
    try:
        if len(request.product_ids) > 50:
            raise HTTPException(
                status_code=400, detail="Maximum 50 products can be published at once"
            )
        ...
        for row in rows:
            ...
            if validation.is_valid or (
                request.force_publish and validation.compliance_level != "fail"
            ):
...
async def get_product_statistics(
    account_type: str | None = Query(
        None, description="Filter by account type: main, fbe, or both"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    try:
        ...
        return {"status": "success", "data": dict(stats) if stats else {}}
...
async def export_products(
    ...
    fields: str | None = Query(
        None, description="Comma-separated list of fields to export"
    ),
    product_ids: str | None = Query(
        None, description="Comma-separated list of product IDs"
    ),
    account_type: str | None = Query(
...
async def update_product_chinese_name(
    product_id: int,
    chinese_name_data: dict[str, str],
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """
    Update product chinese name in local database.

    Args:
        product_id: ID of the local product
        chinese_name_data: Dictionary with 'chinese_name' key
    ...
    try:
        chinese_name = chinese_name_data.get("chinese_name")
        if chinese_name is None:
            raise HTTPException(
                status_code=422, detail="chinese_name is required in request body"
            )
        ...
        # Update chinese name
        old_chinese_name = product.chinese_name
        product.chinese_name = chinese_name if chinese_name.strip() else None
        ...
        return {
            ...
            "data": {
                ...
                "new_chinese_name": product.chinese_name,
            },
        }
...
async def update_product_name(
    ...
    """
    Update product name (Romanian) in local database.

    Args:
        product_id: ID of the local product
        name_data: Dictionary with 'name' key
    ...
    try:
        ...
        if name is None:
            raise HTTPException(
                status_code=422, detail="name is required in request body"
            )
