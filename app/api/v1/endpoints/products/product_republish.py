...
class RepublishProductRequest(BaseModel):
    ...
    new_sku: str = Field(
        ..., description="New SKU for republished product (e.g., EMG331-V2)"
    )
    new_ean: str | None = Field(None, description="New EAN code (if different)")
    new_name: str | None = Field(
        None, description="New product name (slightly different)"
    )
    reason: str = Field(
        ...,
        description="Reason for republishing (e.g., 'Competitor attached to original')",
    )
...
async def create_republished_variant(
    ...
    if request.account_type not in ["main", "fbe"]:
        raise HTTPException(
            status_code=400, detail="account_type must be 'main' or 'fbe'"
        )

    relationship_service = ProductRelationshipService(db)

    # Check if variant group exists
    existing_variants = await relationship_service.get_product_variants(
    ...
    if not existing_variants:
        ...
        variant_group_id = (
            existing_variants[0].get("variant_group_id") if existing_variants else None
        )
        ...
        "data": {
            ...
            "summary": {
                ...
                "primary_variant": next(
                    (v for v in variants if v.get("is_primary")), None
                ),
            },
        },
    }
