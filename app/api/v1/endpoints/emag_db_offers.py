...
from app.api.v1.endpoints.database import (
    get_async_session,  # reuse dependency if available
)

router = APIRouter(tags=["emag-db"])


# Safe dependency to get async DB session
async def get_db_session() -> AsyncSession:
    try:
        # Prefer the core database provider if present
        from app.core.database import (
            get_async_session as core_get_async_session,  # type: ignore
        )

        async for s in core_get_async_session():
            return s
        raise RuntimeError("Could not acquire async DB session from core.database")
    except Exception:
        # Fallback to API endpoint's dependency (works in this codebase)
        async for s in get_async_session():
            return s
        raise RuntimeError(
            "Could not acquire async DB session from api endpoint provider"
        )
...
async def list_offers(
    ...
    search: Optional[str] = Query(None, description="Filter by product name (ILIKE)"),
    min_price: Optional[float] = Query(None, ge=0, description="Minimum sale_price"),
    max_price: Optional[float] = Query(None, ge=0, description="Maximum sale_price"),
    stock_gt: Optional[int] = Query(
