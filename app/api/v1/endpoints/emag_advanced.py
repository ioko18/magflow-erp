...
class LightOfferUpdateRequest(BaseModel):
    ...
    recommended_price: Optional[float] = Field(None, description="Recommended retail price")
    ...
    stock_value: Optional[int] = Field(None, description="Stock quantity")
    warehouse_id: int = Field(1, description="Warehouse ID")
    handling_time_value: Optional[int] = Field(None, description="Handling time in days")
    vat_id: Optional[int] = Field(None, description="VAT rate ID")
    status: Optional[int] = Field(None, description="Offer status (0=inactive, 1=active)")
    ...
    **Note:** Cannot create new offers or modify product information.
    ...
    try:
        ...
        if request.stock_value is not None:
            stock = [{"warehouse_id": request.warehouse_id, "value": request.stock_value}]

        # Build handling_time array if provided
        handling_time = None
        if request.handling_time_value is not None:
            handling_time = [{"warehouse_id": request.warehouse_id, "value": request.handling_time_value}]
    ...
    **Returns:**
    ...
    try:
        if len(request.eans) > 100:
            raise HTTPException(
                status_code=400,
                detail="Maximum 100 EAN codes allowed per request"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to search EANs: {str(e)}"
        )
...
async def get_emag_categories(
    ...
    language: str = Query("ro", description="Response language (en, ro, hu, bg, pl, gr, de)"),
    current_user=Depends(get_current_user),
):
...
async def get_vat_rates(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch VAT rates: {str(e)}"
        )
...
async def get_handling_times(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch handling times: {str(e)}"
        )
...
async def bulk_update_offers_light(
    ...
    batch_size: int = Query(default=25, ge=1, le=50, description="Batch size for processing"),
    current_user=Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    ...
    **Returns:**
    ...
    for account_type, account_updates in updates_by_account.items():
        try:
            ...
            for i in range(0, len(account_updates), batch_size):
                ...
                for update in batch:
                    ...
                    if update.stock_value is not None:
                        stock = [{"warehouse_id": update.warehouse_id, "value": update.stock_value}]

                    # Build handling_time array if provided
                    handling_time = None
                    if update.handling_time_value is not None:
                        handling_time = [{"warehouse_id": update.warehouse_id, "value": update.handling_time_value}]
                ...
                # Process results
                for (product_id, _), result in zip(batch_tasks, batch_results):
            ...
            logger.error(f"Error processing bulk updates for account {account_type}: {e}", exc_info=True)
    ...
    return {
        ...
        "summary": {
            ...
            "failed": total_failed,
            "success_rate": round((total_success / len(updates)) * 100, 2) if updates else 0
        },
        "results": results
    }
