...
class DateInterval(BaseModel):
    ...
    start_date: str = Field(..., description="Start date in format YYYY-MM-DD HH:MM:SS.SSSSSS")
    end_date: str = Field(..., description="End date in format YYYY-MM-DD HH:MM:SS.SSSSSS")
    voucher_discount: int = Field(..., ge=10, le=100, description="Discount percentage for this interval")
...
class CampaignProposalRequest(BaseModel):
    ...
    # Optional fields
    max_qty_per_order: Optional[int] = Field(None, description="Max quantity per order (required for Stock-in-site)")
    voucher_discount: Optional[int] = Field(None, ge=10, le=100, description="Voucher discount percentage")
    post_campaign_sale_price: Optional[float] = Field(None, gt=0, description="Price after campaign ends")
    not_available_post_campaign: bool = Field(False, description="Deactivate offer after campaign")

    # MultiDeals specific
    date_intervals: Optional[List[DateInterval]] = Field(None, description="Date intervals for MultiDeals campaigns")

    @validator('date_intervals')
...
class BulkCampaignProposalRequest(BaseModel):
    ...
    proposals: List[CampaignProposalRequest] = Field(..., max_items=50, description="List of campaign proposals (max 50)")
    ...
    try:
        ...
        async with EmagApiClient(
            ...
            if request.date_intervals:
                date_intervals_emag = [interval.to_emag_format() for interval in request.date_intervals]
            ...
            if result.get("isError"):
                ...
                logger.warning(f"Campaign proposal error for product {request.product_id}: {error_msg}")
        ...
        logger.error(f"Error proposing product {request.product_id} to campaign {request.campaign_id}: {e}")
        return CampaignProposalResponse(
            status="failed",
...
async def bulk_propose_to_campaigns(
    ...
    for account_type, account_proposals in proposals_by_account.items():
        try:
                ...
                for proposal in account_proposals:
                    try:
                        ...
                        if proposal.date_intervals:
                            date_intervals_emag = [interval.to_emag_format() for interval in proposal.date_intervals]
                        ...
                        logger.error(f"Error processing proposal for product {proposal.product_id}: {e}")
                        results.append({
                            "product_id": proposal.product_id,
                            "campaign_id": proposal.campaign_id,
                            "status": "failed",
            ...
            logger.error(f"Error processing bulk proposals for account {account_type}: {e}")
    ...
    return BulkCampaignProposalResponse(
        ...
        summary={
            ...
            "failed": total_failed,
            "success_rate": round((total_success / len(request.proposals)) * 100, 2) if request.proposals else 0
        },
        results=results
    )
