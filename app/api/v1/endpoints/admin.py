...
async def get_dashboard_data(
    ...
    try:
        # ===== SALES METRICS =====
        # Get total sales from emag_orders (current month) - using real data
        ...
        if sales_previous and sales_previous.total_sales > 0:
            sales_growth = ((sales_current.total_sales - sales_previous.total_sales) / sales_previous.total_sales) * 100
        
        orders_growth = 0.0
        if sales_previous and sales_previous.total_orders > 0:
            orders_growth = ((sales_current.total_orders - sales_previous.total_orders) / sales_previous.total_orders) * 100
        
        # ===== CUSTOMER METRICS =====
        # Count unique customers from emag_orders
        ...
        # Get previous month customer count
        ...
        if customers_previous and customers_previous.total_customers > 0:
            customers_growth = ((customers_current.total_customers - customers_previous.total_customers) / customers_previous.total_customers) * 100
        ...
        emag_result = await db.execute(
            text("""
                SELECT
                    ...
                    COUNT(*) FILTER (WHERE account_type = 'main') as main_products,
                    COUNT(*) FILTER (WHERE account_type = 'fbe') as fbe_products
        ...
        if emag_previous and emag_previous.total_products > 0:
            emag_growth = ((emag_data.total_products - emag_previous.total_products) / emag_previous.total_products) * 100
        ...
        recent_orders_result = await db.execute(
            text("""
                SELECT 
                    eo.id,
                    eo.emag_order_id as order_number,
                    eo.customer_name,
                    eo.total_amount,
                    CASE 
                        WHEN eo.status = 0 THEN 'cancelled'
                        WHEN eo.status = 1 THEN 'new'
                        WHEN eo.status = 2 THEN 'in_progress'
                        WHEN eo.status = 3 THEN 'prepared'
                        WHEN eo.status = 4 THEN 'finalized'
                        WHEN eo.status = 5 THEN 'returned'
                        ELSE 'unknown'
                    END as status,
                    eo.order_date,
                    eo.currency
        ...
        inventory_status_result = await db.execute(
            text("""
                SELECT 
                    COALESCE(emag_category_name, 'Uncategorized') as category,
        ...
        realtime_result = await db.execute(
            text("""
                SELECT 
                    (SELECT COUNT(*) FROM app.emag_orders WHERE status IN (1, 2)) as pending_orders,
        ...
        sync_progress = 100
        if emag_data and emag_data.recent_updates > 0:
            sync_progress = min(100, int((emag_data.recent_updates / max(emag_data.total_products, 1)) * 100))
        ...
        return {
            ...
            "data": {
                ...
                "totalCustomers": int(customers_current.total_customers) if customers_current else 0,
                "emagProducts": int(emag_data.total_products) if emag_data else 0,
                "inventoryValue": float(inventory_data.total_value) if inventory_data else 0.0,
                ...
                "realtimeMetrics": {
                    ...
                    "pendingOrders": int(realtime_data.pending_orders) if realtime_data else 0,
                    "lowStockItems": int(realtime_data.low_stock_items) if realtime_data else 0,
                ...
                "recentOrders": [
                    {
                        ...
                        "priority": "high" if float(order.total_amount) > 500 else "medium" if float(order.total_amount) > 200 else "low"
                    }
                ...
                "salesData": [
                    {
                        ...
                        "growth": 0.0  # TODO: Calculate month-over-month growth
                    }
                ...
                "topProducts": [
                    {
                        ...
                        "trend": "up" if product.stock > 20 else "stable" if product.stock > 10 else "down"
                    }
...
async def admin_get_emag_products_by_account(
    ...
    try:
        ...
        for row in product_rows:
            ...
            ean_codes = list(
                set([str(ean).strip() for ean in ean_codes if ean and str(ean).strip()])
            )
...
async def get_emag_orders(
    ...
    channel: Optional[str] = Query(None, description="Filter by channel (main/fbe)"),
    start_date: Optional[str] = Query(None, description="Filter orders from this date"),
    end_date: Optional[str] = Query(None, description="Filter orders until this date"),
    search: Optional[str] = Query(None, description="Search by order number, customer name, email or phone"),
    db: AsyncSession = Depends(get_db),
    current_user=Depends(get_current_user),
):
    """
    ...
    try:
            ...
            formatted_orders.append(
                {
                    ...
                    "last_synced_at": (
                        order.last_synced_at.isoformat() if order.last_synced_at else None
                    ),
                }
            )
        ...
        channel_breakdown = {
            ...
            for row in channel_breakdown_result.fetchall()
        }

        # Get sync status breakdown
        sync_breakdown_query = select(
            EmagOrder.sync_status,
            func.count().label('count')
        ...
        sync_breakdown = {
            ...
            for row in sync_breakdown_result.fetchall()
        }
        ...
        new_orders_24h_query = select(func.count()).select_from(EmagOrder).where(
        ...
        synced_today_query = select(func.count()).select_from(EmagOrder).where(
            EmagOrder.last_synced_at >= today_start
        )
        if filters:
        ...
        # Pending sync orders
        pending_sync_query = select(func.count()).select_from(EmagOrder).where(
            EmagOrder.sync_status.in_(['pending', 'failed'])
        )
        if filters:
...
async def get_unified_products(
    skip: int = Query(0, ge=0, description="Number of records to skip"),
    limit: int = Query(50, ge=1, le=200, description="Number of records to return"),
    marketplace_presence: Optional[str] = Query(
        None, 
    ...
    sync_status: Optional[str] = Query(
        None,
        description="Filter by sync status: synced, pending, failed, never_synced"
    ),
    price_comparison: Optional[str] = Query(
        None,
    ...
    sort_by: Optional[str] = Query(
        "updated_at",
        description="Sort field: name, sku, price, updated_at"
    ),
    ...
    try:
        ...
        for row in product_rows:
            ...
            # Price comparison
            price_info = {
                "local_price": float(row.get("local_price")) if row.get("local_price") else None,
                "emag_main_price": float(row.get("emag_main_price")) if row.get("emag_main_price") else None,
                "emag_fbe_price": float(row.get("emag_fbe_price")) if row.get("emag_fbe_price") else None,
                "has_price_difference": False
            }
            
            # Check for price differences
            prices = [p for p in [price_info["local_price"], price_info["emag_main_price"], price_info["emag_fbe_price"]] if p is not None]
            if len(prices) > 1:
            ...
            formatted_products.append({
                ...
                "local": {
                    ...
                    "updated_at": row.get("local_updated_at").isoformat() if row.get("local_updated_at") else None
                } if row.get("has_local") else None,
                "emag_main": {
                    "id": str(row.get("emag_main_id")) if row.get("emag_main_id") else None,
                    ...
                    "last_synced": row.get("emag_main_last_synced").isoformat() if row.get("emag_main_last_synced") else None,
                    ...
                    "buy_button_rank": int(row.get("emag_main_rank")) if row.get("emag_main_rank") else None,
                    "has_competitors": (row.get("emag_main_offers") or 1) > 1
                } if row.get("has_emag_main") else None,
                "emag_fbe": {
                    "id": str(row.get("emag_fbe_id")) if row.get("emag_fbe_id") else None,
                    ...
                    "last_synced": row.get("emag_fbe_last_synced").isoformat() if row.get("emag_fbe_last_synced") else None,
                    "category": row.get("emag_fbe_category"),
                    "part_number_key": row.get("emag_fbe_pnk"),
                    "number_of_offers": int(row.get("emag_fbe_offers") or 1),
                    "buy_button_rank": int(row.get("emag_fbe_rank")) if row.get("emag_fbe_rank") else None,
                    "has_competitors": (row.get("emag_fbe_offers") or 1) > 1
                } if row.get("has_emag_fbe") else None,
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch unified products: {exc}")
