...
from app.services.suppliers.supplier_import_service import SupplierImportService
...
async def import_products_from_excel(
    ...
    try:
        result = await import_service.import_from_excel(
            file_content=content, supplier_id=supplier_id
        )
        return result
...
async def get_import_batches(
    ...
    ).group_by(SupplierRawProduct.import_batch_id, SupplierRawProduct.supplier_id)
...
async def match_products_by_text(
    ...
    groups = await matching_service.match_products_by_text(threshold=request.threshold)
...
async def match_products_by_image(
    ...
    groups = await matching_service.match_products_by_image(threshold=request.threshold)
...
async def match_products_hybrid(
    ...
    groups = await matching_service.match_products_hybrid(threshold=request.threshold)
...
async def get_matching_groups(
    ...
    stmt = select(ProductMatchingGroup).where(ProductMatchingGroup.is_active)
...
async def get_matching_group_detail(
    ...
    products_stmt = (
        ...
        .where(SupplierRawProduct.product_group_id == group_id)
...
@router.get(
    "/groups/{group_id}/price-comparison", response_model=PriceComparisonResponse
)
...
async def get_matching_statistics(
    ...
    # Count suppliers
    suppliers_stmt = select(
        func.count(func.distinct(SupplierRawProduct.supplier_id))
    ...
    return MatchingStatsResponse(
        ...
        matching_rate=(matched_products / total_products * 100)
        if total_products > 0
        else 0,
    )
...
async def get_raw_products(
    ...
    stmt = select(SupplierRawProduct).where(SupplierRawProduct.is_active)
