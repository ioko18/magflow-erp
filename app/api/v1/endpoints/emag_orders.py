...
class OrderSyncRequest(BaseModel):
    ...
    status_filter: Optional[int] = Field(None, description="Order status filter (1=new, 2=in_progress, etc.)")
    max_pages: int = Field(50, description="Maximum pages to fetch per account")
    days_back: Optional[int] = Field(None, description="Number of days to look back for orders")
    sync_mode: str = Field("incremental", description="Sync mode: incremental (only new/updated), full (all orders), historical (specific date range)")
    start_date: Optional[str] = Field(None, description="Start date for historical sync (YYYY-MM-DD)")
    end_date: Optional[str] = Field(None, description="End date for historical sync (YYYY-MM-DD)")
    auto_acknowledge: bool = Field(False, description="Automatically acknowledge new orders (status 1 -> 2)")
...
class OrderStatusUpdateRequest(BaseModel):
    ...
    products: Optional[List[Dict[str, Any]]] = Field(None, description="Updated products")
...
class InvoiceAttachRequest(BaseModel):
    ...
    invoice_name: Optional[str] = Field(None, description="Display name for invoice")
...
async def sync_orders(
    ...
    try:
        ...
        if request.sync_mode == "incremental":
            ...
            logger.info(f"Historical sync mode: {request.start_date} to {request.end_date}")
        else:  # full
        ...
        if request.account_type == "both":
            ...
            total_synced = results["main"].get("synced", 0) + results["fbe"].get("synced", 0)
            total_created = results["main"].get("created", 0) + results["fbe"].get("created", 0)
            total_updated = results["main"].get("updated", 0) + results["fbe"].get("updated", 0)
...
async def get_all_orders(
    ...
    limit: Optional[int] = Query(None, le=1000, description="Maximum orders to return (deprecated, use items_per_page)"),
    offset: Optional[int] = Query(None, ge=0, description="Offset for pagination (deprecated, use page)"),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
) -> Dict[str, Any]:
    """
            ...
            "orders": [
                    {
                        ...
                        "invoice_uploaded_at": order.invoice_uploaded_at.isoformat() if order.invoice_uploaded_at else None,
...
async def attach_invoice(
    ...
    """
    logger.info(
        "User %s attaching invoice to order %d",
        current_user.email,
        order_id
    )
