...
async def get_stock_sync_alerts(
    ...
    medium_priority = [
        a
        for a in alerts
        if "needs_weekly_update" in a["issues"] and a not in high_priority
    ]
...
async def suggest_stock_transfer(
    ...
    if request.from_account not in ["main", "fbe"]:
        raise HTTPException(
            status_code=400, detail="from_account must be 'main' or 'fbe'"
        )

    if request.to_account not in ["main", "fbe"]:
        raise HTTPException(
            status_code=400, detail="to_account must be 'main' or 'fbe'"
        )

    if request.from_account == request.to_account:
        raise HTTPException(
            status_code=400, detail="from_account and to_account must be different"
        )
...
async def get_stock_sync_dashboard(
    ...
    # Categorize issues
    zero_stock_competition = len(
        [a for a in alerts if "zero_stock_with_competition" in a["issues"]]
    )
