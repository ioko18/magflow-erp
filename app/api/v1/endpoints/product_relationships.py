...
class PNKConsistencyResponse(BaseModel):
    ...
    sku: str
    is_consistent: bool
    pnk_main: Optional[str]
    pnk_fbe: Optional[str]
    status: str
    issues: List[str]
    has_main: bool
    has_fbe: bool
...
class CreateVariantGroupRequest(BaseModel):
    ...
    reason: str = Field(default="Re-published due to competition", description="Reason for variants")
...
class CreateProductFamilyRequest(BaseModel):
    ...
    product_type: str = Field(default="local", description="Product type: local, emag_main, emag_fbe")
...
async def get_relationships_dashboard(
    ...
    return {
        ...
        "data": {
            "pnk_inconsistencies": {
                "total": len(inconsistencies),
                "critical": len([i for i in inconsistencies if i["status"] == "inconsistent"]),
                "missing": len([i for i in inconsistencies if i["status"] == "missing"]),
            },
            "competition": {
                "total_alerts": len(competition_alerts),
                "requires_action": len([a for a in competition_alerts if a.get("action_taken") is None]),
            },
        }
    }
