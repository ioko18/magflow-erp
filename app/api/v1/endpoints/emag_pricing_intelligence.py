...
class SmartDealsCheckResponse(BaseModel):
    ...
    discount_required: Optional[float] = None
...
class PricingRecommendationResponse(BaseModel):
    ...
    smart_deals_target_price: Optional[float] = None
...
@router.get("/commission/estimate/{product_id}", response_model=CommissionEstimateResponse)
    ...
    try:
        ...
        async with EmagApiClient(
            ...
            if result.get("isError"):
                ...
                logger.warning(f"Commission estimate error for product {product_id}: {error_msg}")
...
async def check_smart_deals_eligibility(
    ...
    try:
        ...
        async with EmagApiClient(
            ...
            if result.get("isError"):
                error_msg = result.get("messages", [{}])[0].get("text", "Unknown error")
                logger.warning(f"Smart Deals check error for product {product_id}: {error_msg}")
            ...
            if current_price and target_price and current_price > 0:
                discount_required = ((current_price - target_price) / current_price) * 100
        ...
        logger.error(f"Error checking Smart Deals eligibility for product {product_id}: {e}")
        return SmartDealsCheckResponse(
            product_id=product_id,
            error=str(e)
        )
...
async def get_pricing_recommendations(
    ...
    try:
        ...
        async with EmagApiClient(
            ...
            smart_deals_result = await client.check_smart_deals_eligibility(request.product_id)
            ...
            if not smart_deals_result.get("isError"):
                ...
                if smart_deals_eligible:
                    ...
                    discount_needed = ((request.current_price - smart_deals_target) / request.current_price) * 100
                    recommendations.append(
        ...
        async with EmagApiClient(
            ...
            for product_id in ids:
                try:
                    ...
                    # Get Smart Deals
                    smart_deals_result = await client.check_smart_deals_eligibility(product_id)
                    smart_deals_data = {}
                    if not smart_deals_result.get("isError"):
                        data = smart_deals_result.get("results", {})
                        if isinstance(data, list) and data:
