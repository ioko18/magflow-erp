...
class SupplierRawProduct(Base, TimestampMixin):
    ...
    # Processed Data
    english_name: Mapped[str | None] = mapped_column(String(1000))  # Auto-translated
    normalized_name: Mapped[str | None] = mapped_column(String(1000))  # For matching

    # Image Analysis
    image_hash: Mapped[str | None] = mapped_column(String(64))  # Perceptual hash
    image_features: Mapped[dict | None] = mapped_column(JSON)  # ML features
    ...
    matching_status: Mapped[str] = mapped_column(
        String(20), default=MatchingStatus.PENDING, index=True
    )
    product_group_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("app.product_matching_groups.id"), index=True
    )
    ...
    product_group: Mapped[Optional["ProductMatchingGroup"]] = relationship(
        "ProductMatchingGroup", back_populates="raw_products"
    )
...
class ProductMatchingGroup(Base, TimestampMixin):
    ...
    group_name_en: Mapped[str | None] = mapped_column(String(500))
    description: Mapped[str | None] = mapped_column(Text)

    # Representative Image (from best quality source)
    representative_image_url: Mapped[str | None] = mapped_column(String(2000))
    representative_image_hash: Mapped[str | None] = mapped_column(String(64))
    ...
    matching_method: Mapped[str] = mapped_column(
        String(50)
    )  # text, image, hybrid, manual

    # Status
    status: Mapped[str] = mapped_column(
        String(20), default=MatchingStatus.AUTO_MATCHED, index=True
    )
    verified_by: Mapped[int | None] = mapped_column(Integer)  # User ID
    verified_at: Mapped[datetime | None] = mapped_column(DateTime)

    # Price Analysis
    min_price_cny: Mapped[float | None] = mapped_column(Float)
    max_price_cny: Mapped[float | None] = mapped_column(Float)
    avg_price_cny: Mapped[float | None] = mapped_column(Float)
    best_supplier_id: Mapped[int | None] = mapped_column(
        Integer
    )  # Supplier with best price
    ...
    local_product_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("app.products.id"), index=True
    )
    ...
    raw_products: Mapped[list["SupplierRawProduct"]] = relationship(
        "SupplierRawProduct", back_populates="product_group"
    )
...
class ProductMatchingScore(Base, TimestampMixin):
    ...
    product_a_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("app.supplier_raw_products.id"), nullable=False
    )
    ...
    image_similarity: Mapped[float | None] = mapped_column(Float)
    price_similarity: Mapped[float | None] = mapped_column(Float)
    total_score: Mapped[float] = mapped_column(Float, default=0.0)

    # Matching Details
    matching_algorithm: Mapped[str] = mapped_column(
        String(50)
    )  # cosine, jaccard, perceptual_hash, etc.
    ...
    # Relationships
    product_a = relationship("SupplierRawProduct", foreign_keys=[product_a_id])
...
class SupplierPriceHistory(Base, TimestampMixin):
    ...
    raw_product_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("app.supplier_raw_products.id"), nullable=False, index=True
    )

    # Price Data
    price_cny: Mapped[float] = mapped_column(Float, nullable=False)
    price_change: Mapped[float | None] = mapped_column(Float)  # Change from previous
