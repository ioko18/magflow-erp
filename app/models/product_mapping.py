...
class GoogleSheetsProductMapping(Base, TimestampMixin):
    """
    ...
    local_product_name: Mapped[str] = mapped_column(
        String(500), nullable=False, comment="Romanian product name from Google Sheets"
    )
    local_price: Mapped[float | None] = mapped_column(
        Float, nullable=True, comment="Price from Google Sheets (Emag_FBE_RO_Price_RON)"
    )

    # eMAG MAIN account mapping
    emag_main_id: Mapped[UUID_TYPE | None] = mapped_column(
        UUID(as_uuid=True),
        nullable=True,
        index=True,
        comment="ID from emag_products_v2 table for MAIN account",
    )
    emag_main_part_number: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="Part number from eMAG MAIN account"
    )
    emag_main_status: Mapped[str | None] = mapped_column(
        String(50),
        nullable=True,
    ...
    emag_fbe_id: Mapped[UUID_TYPE | None] = mapped_column(
        UUID(as_uuid=True),
        nullable=True,
        index=True,
        comment="ID from emag_products_v2 table for FBE account",
    )
    emag_fbe_part_number: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="Part number from eMAG FBE account"
    )
    emag_fbe_status: Mapped[str | None] = mapped_column(
        String(50),
        nullable=True,
    ...
    mapping_confidence: Mapped[float | None] = mapped_column(
        Float, nullable=True, comment="Confidence score for automatic mapping (0.0-1.0)"
    )
    ...
    google_sheet_row: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="Row number in Google Sheets"
    )
    google_sheet_data: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="Raw data from Google Sheets (JSON)"
    )
    ...
    notes: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="Additional notes about the mapping"
    )
    has_conflicts: Mapped[bool] = mapped_column(
        Boolean(), default=False, comment="Whether there are conflicts in the mapping"
    )
...
class ImportLog(Base, TimestampMixin):
    ...
    total_rows: Mapped[int] = mapped_column(
        Integer, default=0, comment="Total rows processed"
    )
    successful_imports: Mapped[int] = mapped_column(
        Integer, default=0, comment="Number of successful imports"
    )
    failed_imports: Mapped[int] = mapped_column(
        Integer, default=0, comment="Number of failed imports"
    )
    skipped_rows: Mapped[int] = mapped_column(
        Integer, default=0, comment="Number of skipped rows"
    )

    # Mapping statistics
    auto_mapped_main: Mapped[int] = mapped_column(
        Integer, default=0, comment="Products automatically mapped to MAIN"
    )
    auto_mapped_fbe: Mapped[int] = mapped_column(
        Integer, default=0, comment="Products automatically mapped to FBE"
    )
    unmapped_products: Mapped[int] = mapped_column(
        Integer, default=0, comment="Products that couldn't be mapped"
    )
    ...
    initiated_by: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="User who initiated the import"
    )
