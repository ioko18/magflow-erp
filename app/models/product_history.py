...
class ProductSKUHistory(Base):
    ...
    old_sku: Mapped[str] = mapped_column(
        ...
        nullable=False,
        index=True,
        comment="Previous SKU value"
    )
    new_sku: Mapped[str] = mapped_column(
        String(100),
        nullable=False,
        index=True,
        comment="New SKU value"
    )
    ...
    change_reason: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="Reason for the change (optional)"
    )
...
class ProductChangeLog(Base):
    ...
    def __repr__(self) -> str:
        return f"<ProductChangeLog product_id={self.product_id} field={self.field_name}>"
