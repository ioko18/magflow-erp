...
class EmagCategory(Base):
    ...
    is_allowed = Column(Integer, nullable=False, default=0, comment="1 if seller can post in this category")
    ...
    characteristics = Column(JSONB, nullable=True, comment="Category characteristics with mandatory flags")
    family_types = Column(JSONB, nullable=True, comment="Family types for product variants")

    # Language and metadata
    language = Column(String(5), nullable=False, default="ro", comment="Language code: en, ro, hu, bg, pl, gr, de")

    # Timestamps
    created_at = Column(DateTime, nullable=False, default=datetime.utcnow)
    updated_at = Column(DateTime, nullable=False, default=datetime.utcnow, onupdate=datetime.utcnow)
    last_synced_at = Column(DateTime, nullable=True, comment="Last time synced from eMAG API")
...
class EmagVatRate(Base):
    ...
    # VAT rate info
    name = Column(String(100), nullable=False, comment="VAT rate name (e.g., 'Standard Rate 19%')")
    rate = Column(Float, nullable=False, comment="VAT rate as decimal (e.g., 0.19 for 19%)")
...
class EmagHandlingTime(Base):
    ...
    # Handling time info
    value = Column(Integer, nullable=False, comment="Number of days from order to dispatch")
    ...
    def __repr__(self):
        return f"<EmagHandlingTime(id={self.id}, value={self.value}, name='{self.name}')>"
