...
class TrafficGenerator:
    ...
    async def generate_normal_traffic(self, duration: int = 60, rps: int = 10):
        ...
        logger.info(f"Generating normal traffic for {duration} seconds at {rps} requests per second")
        ...
        while time.time() < end_time:
            ...
            for result in results:
                ...
                if request_count % 100 == 0:
                    avg_latency = (total_latency / (success_count or 1)) if success_count > 0 else 0
                    logger.info(
        ...
        return {
            ...
            "avg_latency_ms": total_latency / (success_count or 1) if success_count > 0 else 0
        }
    
    async def generate_high_latency_traffic(self, duration: int = 300, rps: int = 5):
        """Generate traffic with high latency."""
        logger.info(f"Generating high latency traffic for {duration} seconds at {rps} requests per second")
    ...
    async def generate_high_error_traffic(self, duration: int = 300, rps: int = 10):
        ...
        logger.info(f"Generating high error traffic for {duration} seconds at {rps} requests per second")
...
async def main():
    ...
    try:
        ...
        latency_stats = await traffic_gen.generate_high_latency_traffic(duration=300, rps=5)
        logger.info(f"High latency traffic stats: {latency_stats}")
        
        # Generate high error traffic
        logger.info("\n=== Starting high error traffic generation ===")
        error_stats = await traffic_gen.generate_high_error_traffic(duration=300, rps=10)
