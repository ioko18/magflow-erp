        ...
        if path.exists():
            try:
                ...
                if os.access(str(path), os.W_OK):
                    ...
                    print(f"{YELLOW}⚠  {dir_path}/ - {desc} (readable, not writable){RESET}")
                    State.has_warnings = True
            except Exception as e:
                if required:
...
def check_tls_certificates() -> None:
    ...
    if not all_certs_exist:
        ...
        print("  ./scripts/tls/make-cert.sh pgbouncer \"DNS:pgbouncer,DNS:localhost,IP:127.0.0.1\"")
        print("  ./scripts/tls/make-cert.sh postgres \"DNS:postgres,DNS:localhost,IP:127.0.0.1\"\n")
...
def check_jwt_keys() -> None:
    ...
    if not all_keys_exist:
        ...
        print("  openssl rsa -in jwt-keys/private.pem -pubout -out jwt-keys/public.pem\n")
...
def check_docker_services() -> None:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"{YELLOW}⚠  Could not check running services: {result.stderr.strip()}{RESET}")
            State.has_warnings = True
            
    except subprocess.CalledProcessError as e:
        print(f"{RED}✗  Docker is not running or not installed: {e.stderr.strip()}{RESET}")
    ...
    try:
        ...
        try:
            ...
            if response.status_code == 200 and data.get("status") == "ok":
                ...
                print(f"{YELLOW}⚠  Health check returned non-OK status: {response.status_code}{RESET}")
            ...
            try:
                ...
                if response.status_code == 200 and data.get("status") == "ok":
                    print(f"{GREEN}✓  Health check passed (insecure): {health_url}{RESET}")
                    print("    Note: SSL certificate verification is disabled")
                else:
                    print(f"{YELLOW}⚠  Health check failed: {response.status_code}{RESET}")
                    print(f"    Response: {json.dumps(data, indent=2)}")
                    State.has_warnings = True
        ...
        print(f"{YELLOW}⚠  'requests' package not available. Install with: pip install requests{RESET}")
        State.has_warnings = True
    except Exception as e:
...
def check_database_migrations() -> None:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"{YELLOW}⚠  Could not check database migrations: {result.stderr.strip()}{RESET}")
            State.has_warnings = True
            
    except subprocess.TimeoutExpired:
        print(f"{YELLOW}⚠  Database migration check timed out. Is the database running?{RESET}")
        State.has_warnings = True
    except Exception as e:
...
def print_summary() -> None:
    ...
    if State.has_errors:
        print(f"{RED}✗  There are errors that need to be fixed before running the application.{RESET}")
    elif State.has_warnings:
        print(f"{YELLOW}⚠  There are some warnings, but the application should work.{RESET}")
    else:
...
def main() -> None:
    ...
    if sys.version_info < (3, 11):
        print(f"{RED}✗  Python 3.11 or higher is required (current: {platform.python_version()}){RESET}")
        State.has_errors = True
    else:
        print(f"{GREEN}✓  Python {platform.python_version()} (meets requirements){RESET}")
