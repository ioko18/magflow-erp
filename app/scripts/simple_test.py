...
def test_endpoint(url, expected_status, test_name, method="GET", data=None):
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            ...
            if response.status == 405:
                if "application/json" in content_type or "application/problem+json" in content_type:
                    try:
                        ...
                        if "detail" in response_data and "Method Not Allowed" in response_data["detail"]:
                            print("✅ Valid 405 Method Not Allowed response")
                            return True
                        else:
                            print("❌ FAIL: Missing or invalid 'detail' field in 405 response")
                            return False
                    except json.JSONDecodeError:
                        print("❌ FAIL: Invalid JSON in 405 response")
                        return False
                else:
                    print(f"❌ FAIL: Expected content type to include 'application/json' or 'application/problem+json', got '{content_type}'")
                    return False
            ...
            if expected_status == 401 and response.status == 401:
                if "application/json" in content_type:
                    try:
                        response_data = json.loads(data)
                        if "detail" in response_data and response_data["detail"] == "Not authenticated":
                            print("✅ Valid 401 Unauthorized response")
                            return True
                        else:
                            print("❌ FAIL: Missing or invalid 'detail' field in 401 response")
                            return False
                    except json.JSONDecodeError:
                        print("❌ FAIL: Invalid JSON in 401 response")
                        return False
                else:
                    print(f"❌ FAIL: Expected content type to include 'application/json' for 401, got '{content_type}'")
                    return False
            
            # For all other 4xx/5xx responses, check for application/problem+json or application/json
            if expected_status >= 400 and "application/problem+json" not in content_type and "application/json" not in content_type:
                print(f"❌ FAIL: Expected content type to include 'application/problem+json' or 'application/json' for status {expected_status}, got '{content_type}'")
                return False
        ...
        if e.code == 401:
            if "application/json" in content_type:
                try:
                    ...
                    if "detail" in response_json and response_json["detail"] == "Not authenticated":
                        print("✅ Valid 401 Unauthorized response")
                        return True
                    else:
                        print("❌ FAIL: Missing or invalid 'detail' field in 401 response")
        ...
        # For all other errors, expect application/problem+json
            ...
            print(f"❌ FAIL: Expected content type to include 'application/problem+json', got '{content_type}'")
            return False
...
def main():
    ...
    tests = [
        ...
        (f"{base_url}/auth/login/access-token", 422, "Test Login Validation Error", "POST"),
    ...
    for test_case in tests:
        ...
        if url in test_data:
            result = test_endpoint(url, status_code, test_name, method=method, data=test_data.get(url, {}))
        else:
