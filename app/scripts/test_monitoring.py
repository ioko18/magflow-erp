...
def check_metrics() -> bool:
    ...
    try:
        ...
        if response.status_code == 200 and "http_server_requests_total" in response.text:
            print("✅ Metrics endpoint is working")
            return True
        print(f"❌ Metrics endpoint returned unexpected response: {response.status_code}")
        return False
...
def main():
    ...
    print("Check your Grafana dashboard at http://localhost:3000 to verify the metrics.")
