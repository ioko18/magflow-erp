...
def generate_report(results: List[TestResult], results_dir: Path):
    ...
    for test_name, test_results in tests.items():
        ...
        if prepared and not_prepared:
            ...
            print(f"  RPS: {prep.rps:.1f} vs {no_prep.rps:.1f} "
                  f"({(prep.rps/no_prep.rps - 1)*100:+.1f}%)")
            print(f"  p95 Latency: {prep.p95:.1f}ms vs {no_prep.p95:.1f}ms "
...
def main():
    ...
    parser.add_argument('--results-dir', type=str, default='./load/results',
