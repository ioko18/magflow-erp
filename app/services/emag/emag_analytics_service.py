...
class EmagAnalyticsService(ServiceBase):
    ...
    def __init__(self, context: ServiceContext):
        ...
        self.emag_service: EmagIntegrationService | None = None
    ...
    def _calculate_trend(self, data: list[float]) -> float:
        ...
        try:
            ...
            slope = (
                len(x) * sum(x_i * y_i for x_i, y_i in zip(x, y, strict=False)) - sum(x) * sum(y)
    ...
    async def analyze_duplicates_and_conflicts(
        self,
        account_type: str,
        duplicates_data: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
