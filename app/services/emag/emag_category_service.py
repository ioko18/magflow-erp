...
class EmagCategoryService:
    ...
    def __init__(self, account_type: str = "main"):
        ...
        logger.info("Initialized EmagCategoryService for %s account", account_type)
    ...
    async def get_categories(
        ...
        try:
            logger.info(
                "Fetching categories page %d (language: %s)", current_page, language
            )
            ...
            logger.info("Fetched %d categories", len(response.get("results", [])))
        ...
        try:
            while current_page <= max_pages:
                response = await self.get_categories(
                    current_page=current_page, items_per_page=100, language=language
                )
    ...
    async def get_allowed_categories(
        ...
        allowed = [cat for cat in all_categories if cat.get("is_allowed") == 1]
