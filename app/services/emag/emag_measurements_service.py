...
class EmagMeasurementsService:
    ...
    def __init__(self, account_type: str = "main"):
        ...
        logger.info("Initialized EmagMeasurementsService for %s account", account_type)
    ...
    async def bulk_save_measurements(
        self, measurements_list: list[dict[str, Any]], batch_size: int = 25
    ) -> dict[str, Any]:
        """
            ...
            for item in batch:
                try:
                    ...
                    results["errors"].append(
                        {"product_id": item.get("id", "unknown"), "error": str(e)}
                    )
    ...
    def calculate_volume_cm3(
        self, length_mm: float, width_mm: float, height_mm: float
    ) -> float:
        """
    ...
    def calculate_volumetric_weight_kg(
        self, length_mm: float, width_mm: float, height_mm: float, divisor: int = 5000
    ) -> float:
        """
