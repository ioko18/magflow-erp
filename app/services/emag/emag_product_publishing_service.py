...
class EmagProductPublishingService:
    ...
    def __init__(self, account_type: str = "main"):
        ...
        logger.info(
            "Initialized EmagProductPublishingService for %s account", account_type
        )
    ...
    async def create_draft_product(
        ...
        try:
            logger.info("Creating draft product %d: %s", product_id, name)

            start_time = time.time()
            response = await self.client._request(
                "POST", "product_offer/save", json=payload
            )
    ...
    async def create_complete_product(
        ...
        images_overwrite: int | None = None,
    ) -> dict[str, Any]:
        """
        ...
        try:
            logger.info("Creating complete product %d: %s", product_id, name)
    ...
    async def attach_offer_to_existing_product(
        ...
        try:
            logger.info("Attaching offer %d to product %s", product_id, part_number_key)
            ...
            logger.info(
                "Offer %d attached successfully to %s", product_id, part_number_key
            )
            return response
    ...
    async def attach_offer_by_ean(
        ...
        try:
            logger.info("Attaching offer %d by EAN: %s", product_id, ", ".join(ean))
