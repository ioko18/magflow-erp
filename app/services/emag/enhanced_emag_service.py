...
from app.config.emag_config import EmagApiConfig, get_emag_config
...
class EnhancedEmagIntegrationService:
        ...
        try:
            ...
            async with async_session_factory() as fbe_session:
                ...
                try:
                    results[
                        "fbe_account"
                    ] = await fbe_service._sync_products_from_account(
        ...
        while True:
            try:
                ...
                if not response or "results" not in response:
                    logger.warning(
                        "No results in response for page %d. Response: %s",
                        page,
                        response,
                    )
                    break
    ...
    async def _process_and_save_products(
                ...
                async with async_session_factory() as product_session:
                    try:
                        ...
                        if existing_product:
                            ...
                            # Create new product
                            new_product = self._create_product_from_emag_data(
                                product_data
                            )
        ...
        success_count = len(
            [p for p in processed_products if p.get("status") == "processed"]
        )
        logger.info("Successfully saved %d products to database", success_count)
    ...
    def _create_product_from_emag_data(
        ...
        elif "offer_details" in product_data and isinstance(
            product_data["offer_details"], dict
        ):
        ...
        if images:
            ...
            if main_images:
                ...
                logger.warning(
                    f"No main image found for product {product_data.get('id')}"
                )
        ...
        if "validation_status" in product_data and isinstance(
            product_data["validation_status"], dict
        ):
            validation_errors = product_data["validation_status"].get("errors", [])

        if "translation_validation_status" in product_data and isinstance(
            product_data["translation_validation_status"], dict
        ):
            translation_validation_errors = product_data[
        ...
        return EmagProductV2(
            ...
            best_offer_recommended_price=self._safe_float(
                product_data.get("best_offer_recommended_price")
            ),
            ...
            genius_eligibility=self._safe_int(
                product_data.get("genius_eligibility", 0)
            ),
            genius_eligibility_type=self._safe_int(
                product_data.get("genius_eligibility_type")
            ),
            genius_computed=self._safe_int(product_data.get("genius_computed", 0)),
            # v4.4.9 - Product Family
            family_id=self._safe_int(product_data.get("family", {}).get("id"))
            if isinstance(product_data.get("family"), dict)
            else None,
            family_name=self._safe_str(product_data.get("family", {}).get("name"))
            if isinstance(product_data.get("family"), dict)
            else None,
            family_type_id=self._safe_int(
                product_data.get("family", {}).get("family_type_id")
            )
            if isinstance(product_data.get("family"), dict)
            else None,
            ...
            translation_validation_errors=translation_validation_errors
            if translation_validation_errors
            else None,
            main_image_url=main_image_url,
            images_validated=images_validated,
            characteristics_validated=bool(
                characteristics
    ...
    def _update_product_from_emag_data(
        ...
        product.number_of_offers = self._safe_int(
            product_data.get("number_of_offers", 0)
        )
        product.buy_button_rank = self._safe_int(product_data.get("buy_button_rank"))
        product.best_offer_sale_price = self._safe_float(
            product_data.get("best_offer_sale_price")
        )
        product.best_offer_recommended_price = self._safe_float(
            product_data.get("best_offer_recommended_price")
        )
        ...
        product.genius_eligibility = self._safe_int(
            product_data.get("genius_eligibility", 0)
        )
        product.genius_eligibility_type = self._safe_int(
            product_data.get("genius_eligibility_type")
        )
        ...
        if isinstance(product_data.get("family"), dict):
            ...
            product.family_type_id = self._safe_int(
                product_data["family"].get("family_type_id")
            )
        ...
        product.force_images_download = bool(
            product_data.get("force_images_download", 0)
        )
        product.attachments = (
            product_data.get("attachments", [])
            if isinstance(product_data.get("attachments"), list)
                ...
                if latest_sync:
                    return {
                        ...
                        "latest_sync": {
                            ...
                            "completed_at": latest_sync.completed_at.isoformat()
    ...
    async def sync_all_orders_from_both_accounts(
        ...
        try:
            ...
            # Combine results
            total_orders = results.get("main_account", {}).get(
                "orders_count", 0
