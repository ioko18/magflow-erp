...
class ProductRelationshipService:
    ...
    async def check_pnk_consistency(self, sku: str) -> Dict[str, Any]:
        ...
        tracking_query = select(ProductPNKTracking).where(
            ProductPNKTracking.sku == sku
        )
    ...
    async def get_pnk_inconsistencies(self, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        query = select(ProductPNKTracking).where(
            ProductPNKTracking.is_consistent is False
        ).order_by(ProductPNKTracking.last_checked.desc()).limit(limit)
    ...
    async def check_competition(self, product_id: UUID, account_type: str) -> Dict[str, Any]:
        """
        ...
        prev_log_query = select(ProductCompetitionLog).where(
            and_(
                ProductCompetitionLog.emag_product_id == product_id,
                ProductCompetitionLog.account_type == account_type
            )
        ).order_by(ProductCompetitionLog.detected_at.desc()).limit(1)
        ...
        if has_competitors:
            if number_of_offers >= 5:
                ...
                requires_action = True
                recommendation = f"{new_competitors} competitori noi. Monitorizați situația."
    ...
    async def get_products_with_competition(self, limit: int = 50) -> List[Dict[str, Any]]:
        """Get products that have competitors and may require action."""
        query = select(ProductCompetitionLog).where(
            ProductCompetitionLog.requires_action is True
        ).order_by(ProductCompetitionLog.detected_at.desc()).limit(limit)
    ...
    async def get_product_variants(self, sku: str) -> List[Dict[str, Any]]:
        ...
        group_query = select(ProductVariant).where(
            ProductVariant.variant_group_id == variant.variant_group_id
    ...
    async def add_product_to_family(
        ...
        parent.superseded_at = datetime.utcnow()
    ...
    async def get_product_family_tree(self, sku: str) -> Dict[str, Any]:
        ...
        family_query = select(ProductGenealogy).where(
            ProductGenealogy.family_id == product.family_id
        ...
        for member in family_members:
            ...
            tree["generations"][gen].append({
                ...
                "superseded_at": member.superseded_at.isoformat() if member.superseded_at else None,
