...
class EmagEANMatchingService:
    ...
    async def initialize(self):
        ...
        logger.info(
            "Initialized eMAG EAN matching service for %s account",
            self.account_type
        )
    ...
    async def find_products_by_ean(
        self,
        ean: str
    ) -> Dict[str, Any]:
    ...
    async def bulk_find_products_by_eans(
        self,
        eans: List[str]
    ) -> Dict[str, Any]:
    ...
    async def _find_local_product_by_ean(
        self,
        ean: str
        ...
        async with async_session_factory() as session:
            ...
            stmt = select(EmagProductV2).where(
                EmagProductV2.attributes.contains({"ean": ean})
            ).limit(1)
    ...
    async def validate_ean_format(self, ean: str) -> Dict[str, Any]:
        ...
        if not ean.isdigit():
            return {
                "valid": False,
                "ean": ean,
                "error": "EAN must contain only digits"
            }
