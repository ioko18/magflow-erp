...
def run_async(coro):
    ...
    try:
        ...
        # Re-raise RuntimeError for event loop issues
    ...
    async with async_session_factory() as db:
        for account_type in ["main", "fbe"]:
            try:
                ...
                logger.error(
                    f"Failed to sync {account_type} orders: {e}", exc_info=True
                )
                results["accounts"][account_type] = {"success": False, "error": str(e)}
...
def sync_emag_products_task(
    self, account_type: str = "both", max_pages_per_account: int = 5
) -> dict[str, Any]:
    """
...
async def _sync_products_async(
    account_type: str, max_pages_per_account: int
) -> dict[str, Any]:
    """
            ...
            try:
                async with EmagOrderService(account_type, db) as order_service:
                    ...
                    query = select(EmagOrder).where(
                        EmagOrder.account_type == account_type, EmagOrder.status == 1
                    )
                ...
                logger.error(
                    f"Failed to acknowledge {account_type} orders: {e}", exc_info=True
                )
                results["accounts"][account_type] = {"success": False, "error": str(e)}
                results["errors"].append(f"{account_type}: {str(e)}")
...
async def _cleanup_logs_async(days_to_keep: int) -> dict[str, Any]:
    ...
    async with async_session_factory() as db:
        try:
            ...
            stmt = delete(EmagSyncLog).where(EmagSyncLog.created_at < cutoff_date)
