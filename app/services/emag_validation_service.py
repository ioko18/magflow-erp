...
class EmagValidationService:
    ...
    def validate_product_complete(
        ...
        # Validate EAN codes
        ean_valid, ean_errors = self.validate_ean_codes(
    ...
    def validate_images(
        ...
        if len(main_images) == 0:
            errors.append("No main image (display_type=1) found. Exactly one main image is required.")
        elif len(main_images) > 1:
            errors.append(f"Multiple main images found ({len(main_images)}). Only one main image is allowed.")
        ...
        for idx, img in enumerate(images):
            ...
            if not url.startswith(('http://', 'https://')):
                errors.append(f"Image {idx + 1}: Invalid URL format. Must start with http:// or https://")
    ...
    def validate_ean_codes(
        ...
        for ean in ean_codes:
            ...
            if not self.EAN_PATTERN.match(ean_clean):
                errors.append(
                    f"Invalid EAN format: '{ean}'. "
                    f"Must be 6-14 numeric characters."
                )
                continue
    ...
    def validate_characteristics(
        ...
        template_lookup = {
            char['id']: char for char in template_chars
        }
        ...
        for field, value in measurements.items():
            ...
            try:
                ...
                if val < 0 or val > 999999:
                    errors.append(
                        f"{field} must be between 0 and 999,999, got {val}"
                    )
    ...
    def _validate_characteristic_format(
        ...
        if type_id == 1:  # Numeric
            ...
            if not re.match(r'^\d+(\.\d+)?\s+\w+$', value_str):
                return False, f"{char_name}: Must be 'number unit' format (e.g., '30 cm'), got '{value}'"
            ...
            if not re.match(r'^\d+\s*x\s*\d+$', value_str, re.IGNORECASE):
                return False, f"{char_name}: Must be 'Width x Height' format (e.g., '1920 x 1080'), got '{value}'"

        elif type_id == 40:  # Volume
            if not re.match(r'^\d+\s*x\s*\d+\s*x\s*\d+(\s*-\s*\d+)?$', value_str, re.IGNORECASE):
                return False, f"{char_name}: Must be 'W x H x D' or 'W x H x D - D2' format, got '{value}'"
