...
class ProductMatchingService:
    ...
    def calculate_text_similarity(self, text1: str, text2: str) -> float:
        ...
        # Weighted average
        similarity = (jaccard * 0.4 + bigram * 0.4 + trigram * 0.2)

        return similarity

    # ==================== IMAGE SIMILARITY ====================

    def calculate_image_hash(self, image_url: str) -> Optional[str]:
        """Calculate perceptual hash for image.
        
        Note: This is a placeholder. In production, you would:
        1. Download the image
        2. Use a library like imagehash or PIL
        3. Calculate perceptual hash (pHash, dHash, etc.)
        """
        # Placeholder: return URL hash for now
        # TODO: Implement actual perceptual hashing
        return hashlib.md5(image_url.encode()).hexdigest()

    def calculate_image_similarity(self, hash1: Optional[str], hash2: Optional[str]) -> float:
        """Calculate similarity between two image hashes.
        
        Note: This is a placeholder. In production, you would:
        1. Use Hamming distance for perceptual hashes
        2. Or use deep learning features (ResNet, etc.)
        """
        if not hash1 or not hash2:
            return 0.0

        # Placeholder: exact match for now
        # TODO: Implement Hamming distance for perceptual hashes
        ...
        for i, product_a in enumerate(products):
            ...
            for product_b in products[i+1:]:
                ...
                similarity = self.calculate_text_similarity(
                    product_a.chinese_name,
                    product_b.chinese_name
                )
    ...
    async def match_products_by_image(
        ...
        groups = []
        for image_hash, group_products in hash_groups.items():
            if len(group_products) > 1:
            ...
            for product_b in products[i+1:]:
                ...
                if product_a.image_hash and product_b.image_hash:
                    image_sim = self.calculate_image_similarity(
                        product_a.image_hash,
                        product_b.image_hash
                    )
                ...
                if hybrid_score >= threshold:
                    ...
                    score = ProductMatchingScore(
                        ...
                        matching_features={
                            ...
                            "has_image": bool(product_a.image_hash and product_b.image_hash)
                        }
                    )
                    self.db.add(score)
    ...
    async def get_best_price_for_group(
        self,
        group_id: int
    ) -> Optional[Tuple[SupplierRawProduct, Supplier]]:
        """Get the product with best price in a group."""

        stmt = select(SupplierRawProduct, Supplier).join(
            Supplier,
