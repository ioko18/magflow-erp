...
from app.models.supplier_matching import SupplierRawProduct, MatchingStatus
...
class SupplierImportService:
    ...
    async def import_from_excel(
        ...
        batch_id = f"import_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:8]}"
    ...
    async def get_import_statistics(self, batch_id: str) -> Dict:
        ...
        total = len(products)
        matched = sum(1 for p in products if p.matching_status != MatchingStatus.PENDING)
    ...
    async def get_supplier_products_summary(self) -> List[Dict]:
        ...
        for supplier in suppliers:
            ...
            matched = sum(
                1 for p in products
                if p.matching_status != MatchingStatus.PENDING
            )
