...
class EmagOrderService:
    ...
    async def initialize(self):
        ...
        logger.info(
            "Initialized eMAG order service for %s account",
            self.account_type
        )
        ...
        while page <= max_pages:
            try:
                ...
                if len(page_orders) < 100:
                    logger.info("Last page detected: only %d orders (less than 100)", len(page_orders))
                    break
        ...
        async with async_session_factory() as session:
            try:
                ...
                order_dict = {
                    ...
                    "status_name": ORDER_STATUS.get(order_data.get("status"), "unknown"),
                    ...
                    "payment_method": PAYMENT_METHODS.get(
                        order_data.get("payment_mode_id"),
                        "unknown"
                    ),
    ...
    async def acknowledge_order(self, order_id: int) -> Dict[str, Any]:
        ...
        """
        logger.info("Acknowledging order %d from %s account", order_id, self.account_type)
