...
from app.models.emag_models import EmagSyncLog, EmagProductV2
...
class MetricsCollector:
    ...
    async def record_request(self, response_time: float, success: bool):
        ...
        async with self._lock:
            now = datetime.utcnow()
            self.requests.append({"time": now, "response_time": response_time, "success": success})
...
class EmagMonitoringService:
    ...
    async def _check_alerts(self):
        ...
        self.alerts["slow_response"] = (
            self.metrics["average_response_time"] > MonitoringThresholds.SLOW_RESPONSE_MS
        )
        ...
        new_alerts = {
            key: value for key, value in self.alerts.items()
    ...
    async def get_sync_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Total syncs
            result = await self.db_session.execute(
                select(func.count(EmagSyncLog.id))
                .where(EmagSyncLog.started_at >= yesterday)
            )
            ...
            # Average duration
            result = await self.db_session.execute(
                select(func.avg(EmagSyncLog.duration_seconds))
                .where(
                    and_(
                        EmagSyncLog.started_at >= yesterday,
                        EmagSyncLog.status == "completed"
            ...
            result = await self.db_session.execute(
                select(func.sum(EmagSyncLog.processed_items))
                .where(EmagSyncLog.started_at >= yesterday)
            )
    ...
    async def get_product_statistics(self) -> Dict[str, Any]:
        ...
        try:
            # Total products
            result = await self.db_session.execute(
                select(func.count(EmagProductV2.id))
            )
            ...
            result = await self.db_session.execute(
                select(func.count(EmagProductV2.id))
                .where(EmagProductV2.last_synced_at >= yesterday)
            )
