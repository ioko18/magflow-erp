...
class ImageSimilarityMatcher:
    ...
    async def compare_images(
        self, scraped_image_url: str, local_images: list[str]
    ) -> float:
...
class AttributeMatcher:
    ...
    def compare_attributes(
        self, scraped_specs: dict[str, Any], local_specs: dict[str, Any]
    ) -> float:
...
class ProductMatchingService:
    ...
    async def find_matches_for_supplier(
        self, supplier_id: int, confidence_threshold: float = 0.5, limit: int = 50
        ...
        for supplier_product in supplier_products:
            if (
                not supplier_product.manual_confirmed
            ):  # Only process unconfirmed matches
                # Find best matches for this supplier product
                best_matches = await self._find_best_matches(
                    supplier_product, local_products, limit=5
                )
    ...
    async def _find_best_matches(
        ...
        for local_product in local_products:
            ...
            text_score = await self.text_matcher.compare_names(
                supplier_product.supplier_product_name, local_product.name
            )

            image_score = await self.image_matcher.compare_images(
                supplier_product.supplier_image_url,
                list(local_product.images.values())
                if local_product.images
                else [],
            )
    ...
    async def auto_match_products_for_supplier(
        self, supplier_id: int, auto_confirm_threshold: float = 0.8
    ) -> dict[str, Any]:
        ...
        for supplier_product in unconfirmed_products:
            ...
            best_matches = await self._find_best_matches(
                supplier_product, local_products, limit=1
            )

            if (
                best_matches
                and best_matches[0].confidence_score >= auto_confirm_threshold
            ):
                # Auto-confirm high confidence matches
                best_match = best_matches[0]
                best_match.manual_confirmed = True
                best_match.confirmed_at = datetime.now(UTC)
                confirmed_count += 1
                logger.info(
                    f"Auto-confirmed match for supplier product {supplier_product.id}"
                )
            else:
    ...
    async def get_matching_statistics(
        ...
        return {
            ...
            "confirmation_rate": round(confirmed_matches / total_matches * 100, 2)
            if total_matches > 0
            else 0,
        }
