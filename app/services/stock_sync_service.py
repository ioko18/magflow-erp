...
class StockSyncService:
    ...
    async def analyze_stock_distribution(self, sku: str) -> Dict[str, Any]:
        ...
        if product_main and product_main.last_synced_at:
            days_since_sync = (datetime.utcnow() - product_main.last_synced_at).days
        ...
        if product_fbe and product_fbe.last_synced_at:
            days_since_sync = (datetime.utcnow() - product_fbe.last_synced_at).days
            if days_since_sync >= 7:
                recommendations.append({
                    "type": "offer_update",
                    "account": "fbe",
    ...
    async def get_products_needing_stock_sync(self, limit: int = 50) -> List[Dict[str, Any]]:
        """
        ...
        query = select(EmagProductV2).where(
            or_(
                ...
                EmagProductV2.last_synced_at < datetime.utcnow() - timedelta(days=7)
            )
        ).limit(limit)
        ...
        for product in products:
            ...
            if product.last_synced_at and (datetime.utcnow() - product.last_synced_at).days >= 7:
    ...
    async def suggest_stock_transfer(
        ...
        query_from = select(EmagProductV2).where(
            and_(
                EmagProductV2.sku == sku,
                EmagProductV2.account_type == from_account
            )
        )
        query_to = select(EmagProductV2).where(
            and_(
                EmagProductV2.sku == sku,
                EmagProductV2.account_type == to_account
            )
        )
        ...
        if new_stock_from == 0 and product_from.number_of_offers > 1:
            warnings.append(f"Transfer will leave {from_account} with 0 stock and competition")
        
        if new_stock_from < 3 and product_from.number_of_offers > 1:
            warnings.append(f"Transfer will leave {from_account} with low stock ({new_stock_from}) and competition")
        ...
        return {
            ...
            "impact": {
                "from_account_impact": "Will lose buy button" if new_stock_from == 0 and product_from.number_of_offers > 1 else "Safe",
                "to_account_impact": "Will gain/maintain buy button" if new_stock_to > 0 else "Neutral"
            },
            "warnings": warnings,
