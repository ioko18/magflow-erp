...
class BackupService:
        ...
        return [
            {
                ...
                "last_synced_at": offer.last_synced_at.isoformat() if offer.last_synced_at else None,
    ...
    async def list_backups(self) -> List[Dict[str, Any]]:
        ...
        for backup_file in sorted(self.backup_dir.glob("emag_backup_*.json*"), reverse=True):
            try:
                timestamp_str = backup_file.stem.replace("emag_backup_", "")
                file_time = datetime.strptime(timestamp_str, "%Y%m%d_%H%M%S")
                file_size = backup_file.stat().st_size
