...
class ProductVariant(Base):
    ...
    emag_product_id = Column(
        UUID(as_uuid=True), ForeignKey("app.emag_products_v2.id"), nullable=True
    )
    ...
    )  # original, republished, competitor_hijacked, variation
    variant_reason = Column(Text, nullable=True)  # Why this variant was created
    is_primary = Column(
        Boolean, nullable=False, default=False
    )  # Is this the main/original variant?
    ...
    parent_variant_id = Column(
        UUID(as_uuid=True), ForeignKey("app.product_variants.id"), nullable=True
    )
    ...
    parent_variant = relationship(
        "ProductVariant", remote_side=[id], backref="child_variants"
    )
...
class ProductPNKTracking(Base):
    ...
    is_consistent = Column(
        Boolean, nullable=False, default=True
    )  # PNK matches between MAIN and FBE
...
class ProductCompetitionLog(Base):
    ...
    )  # How many new competitors since last check

    # Status
    requires_action = Column(
        Boolean, nullable=False, default=False
    )  # Should you re-publish?
    action_taken = Column(
        String(100), nullable=True
    )  # republished, price_adjusted, ignored
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "account_type IN ('main', 'fbe')", name="ck_competition_log_account"
        ),
        {"schema": "app"},
    )
...
class ProductGenealogy(Base):
    ...
    )  # 1=original, 2=first republish, etc.
    parent_id = Column(
        UUID(as_uuid=True), ForeignKey("app.product_genealogy.id"), nullable=True
    )
    is_root = Column(
        Boolean, nullable=False, default=False
    ...
    __table_args__ = (
        ...
        CheckConstraint("generation >= 1", name="ck_genealogy_generation"),
        {"schema": "app"},
    )
