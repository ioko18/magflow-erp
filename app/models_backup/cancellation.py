...
class CancellationRequest(Base, TimestampMixin):
    ...
    emag_order_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
    ...
    stock_restored_at: Mapped[datetime | None] = mapped_column(
        DateTime,
        nullable=True,
    )

    # eMAG integration
    emag_cancellation_id: Mapped[str | None] = mapped_column(
        String(100),
        nullable=True,
    )
    ...
    customer_communication: Mapped[str | None] = mapped_column(Text, nullable=True)

    # Items affected by cancellation
    affected_items: Mapped[str | None] = mapped_column(
        Text,
        nullable=True,
...
class CancellationItem(Base, TimestampMixin):
    ...
    # eMAG integration
    emag_item_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
...
class CancellationRefund(Base, TimestampMixin):
    ...
    # Related entities
    cancellation_request_id: Mapped[int | None] = mapped_column(
        Integer,
        ForeignKey("app.cancellation_requests.id"),
        nullable=True,
    )
...
class EmagCancellationIntegration(Base, TimestampMixin):
    ...
    # eMAG response data
    emag_response: Mapped[str | None] = mapped_column(
        Text,
        nullable=True,
